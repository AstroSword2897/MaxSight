...
def setup_logging(
    log_level: str = "INFO", log_file: Path | None = None, log_dir: Path = Path("logs")
) -> logging.Logger:
    ...
    def write(self, text: str):
        ...
        if text and text.strip():
            ...
            raise PrintGuardViolation(
                "Direct print() usage is forbidden in patient mode. "
                "Use safe_print() or logging instead."
            )
...
def safe_print(message: str, level: str = "INFO", patient_mode: bool = False):
...
_global_guard: PatientPrintGuard | None = None
