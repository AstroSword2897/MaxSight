...
class SchemaDowngrader:
    ...
    def downgrade(
        self, outputs: dict[str, Any], reason: str = "validation_failed"
    ) -> dict[str, Any]:
    ...
    def downgrade_on_missing_heads(
        self, outputs: dict[str, Any], missing_heads: list[str]
    ) -> dict[str, Any]:
...
def validate_and_downgrade(
    outputs: dict[str, Any], strict: bool = True, auto_downgrade: bool = True
...
class SchemaStressTester:
                ...
                if "confidence_sources" in det:
                    ...
                    for missing_head in missing_heads:
                        if missing_head in sources:
                            errors.append(
                                f"Missing head '{missing_head}' still present in confidence_sources"
                            )
    ...
    def test_semantic_drift_over_time(
        self, outputs_sequence: list[dict[str, Any]], stable_scene: bool = True
    ) -> SchemaStressTestResult:
    ...
    def run_all_tests(
        self, outputs: dict[str, Any], outputs_sequence: list[dict[str, Any]] | None = None
