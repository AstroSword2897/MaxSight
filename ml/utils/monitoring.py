...
class PredictionLog:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "correct": self.prediction == self.ground_truth
...
class PredictionMonitor:
    ...
    def _add_alert(
        ...
        recent_alerts = [a for a in self.alerts if a.timestamp > cutoff and a.category == category]
    ...
    def get_current_metrics(self) -> dict:
        ...
        with self._lock:
            ...
            return {
                ...
                "fallback_rate": sum(1 for p in self.predictions if p.used_fallback)
    ...
    def get_alerts(self, severity: AlertSeverity | None = None, hours: int = 24) -> list[Alert]:
        """Get recent alerts."""
...
class ReadinessMonitor(PredictionMonitor):
    ...
    def __init__(
        self,
        window_size: int = 100,
        alert_threshold: dict[str, float] | None = None,
        **kwargs: Any,
    ):
...
class ReadinessDashboard:
    ...
    def __init__(self):
        ...
        self.last_assessment: datetime | None = None
    ...
    def _assess_dataset(self, stats: dict):
        ...
        self.checklist.append(
            ChecklistItem(
                ...
                category="Dataset",
                status="pass"
                if num_impairments >= 10
                else "warning"
                if num_impairments >= 5
                else "fail",
    ...
    def _assess_architecture(self, model: torch.nn.Module, metrics: dict):
        ...
        self.checklist.append(
            ChecklistItem(
                ...
                value=weight_decay,
                recommendation="Add weight decay to prevent overfitting"
                if weight_decay == 0
                else None,
            )
        )
    ...
    def _assess_class_balance(self, stats: dict):
        ...
        self.checklist.append(
            ChecklistItem(
                ...
                recommendation="Oversample high-urgency cases"
                if high_urgency_ratio < 0.1
                else None,
            )
        )
...
def create_monitoring_pipeline(model: torch.nn.Module) -> dict:
    ...
    return {"monitor": PredictionMonitor(), "dashboard": ReadinessDashboard()}
...
class HealthChecker:
    ...
    def check_tier1_heads(self) -> dict[str, Any]:
        ...
        results = {"status": "PASS", "checks": {}, "failures": []}
        ...
        if "objectness" in outputs:
            ...
            if detection_rate < self.TIER1_DETECTION_RATE_THRESHOLD:
                ...
                results["failures"].append(
                    f"Objectness detection rate {detection_rate:.2%} < {self.TIER1_DETECTION_RATE_THRESHOLD:.2%}"
                )
        else:
            results["status"] = "FAIL"
        ...
        if "classifications" in outputs:
            ...
            if avg_confidence < self.TIER1_MAP_THRESHOLD:
                results["status"] = "FAIL"
                results["failures"].append(
                    f"Classification confidence {avg_confidence:.2%} < {self.TIER1_MAP_THRESHOLD:.2%}"
                )
        else:
            results["status"] = "FAIL"
        ...
        if "boxes" in outputs:
            ...
            if valid_boxes < self.TIER1_IOU_THRESHOLD:
                results["status"] = "FAIL"
                results["failures"].append(
                    f"Valid box rate {valid_boxes:.2%} < {self.TIER1_IOU_THRESHOLD:.2%}"
                )
        else:
            results["status"] = "FAIL"
    ...
    def check_latency(self, num_runs: int = 10) -> dict[str, Any]:
        ...
        if avg_latency > self.STAGE_A_LATENCY_THRESHOLD_MS:
            results["status"] = "FAIL"
            results["failures"] = [
                f"Average latency {avg_latency:.1f}ms > {self.STAGE_A_LATENCY_THRESHOLD_MS}ms"
            ]
    ...
    def check_model_integrity(self) -> dict[str, Any]:
        ...
        results: dict[str, Any] = {"status": "PASS", "checks": {}, "failures": []}
        ...
        for name, param in self.model.named_parameters():
            ...
            if param.abs().max() > 1e6:
                ...
                results["warnings"].append(
                    f"Extreme values in {name}: max={param.abs().max().item():.2e}"
                )
