...
class MultiHeadBenchmark:
            ...
            if mean_latency > target_latency_ms:
                ...
                if len(combination.split("+")) > 8:
                    recommendations.append(
                        f"Too many heads in {combination}, consider disabling optional heads"
                    )
    ...
    def save_results(self, filepath: Path):
        ...
        with open(filepath, "w") as f:
            json.dump(
                {"results": self.results, "bottleneck_analysis": self.identify_bottlenecks()},
                f,
                indent=2,
            )

    def get_optimal_head_config(
        self, target_latency_ms: float = 500.0, required_heads: list[str] | None = None
    ) -> list[str]:
