...
class PerClassMetricsTracker:
    ...
    def __init__(
        ...
        impairment_names: list[str] | None = None,
        ...
        # Per-class metrics.
        self.class_metrics: dict[str, ClassMetrics] = {name: ClassMetrics() for name in class_names}
        ...
        self.impairment_metrics: dict[str, dict[str, ClassMetrics]] = {
            imp: {name: ClassMetrics() for name in class_names} for imp in self.impairment_names
        }
    ...
    def update(
        ...
        for i in range(batch_size):
            ...
            target_name = (
                self.class_names[target] if target < len(self.class_names) else f"class_{target}"
            )
    ...
    def get_summary(self) -> dict:
        ...
        macro_precision = np.mean(
            [m.precision for m in self.class_metrics.values() if m.support > 0]
        )
        ...
        return {
            "aggregate": {
                ...
                "accuracy": 1
                - len(self.misclassifications_log) / max(len(self.predictions_log), 1),
            },
...
def compute_map_per_class(
    predictions: list[dict], targets: list[dict], iou_threshold: float = 0.5
) -> dict[str, float]:
    ...
    for img_idx, (pred, target) in enumerate(zip(predictions, targets)):
        ...
        for box, label, score in zip(pred_boxes, pred_labels, pred_scores):
            class_predictions[label].append({"box": box, "score": score, "img_idx": img_idx})

        for box, label in zip(target_boxes, target_labels):
            class_targets[label]["boxes"].append({"box": box, "img_idx": img_idx})
    ...
    for class_id in set(class_predictions.keys()) | set(class_targets.keys()):
        preds = sorted(class_predictions[class_id], key=lambda x: x["score"], reverse=True)
