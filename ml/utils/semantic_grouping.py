...
class SemanticGrouper:
    ...
    def group_objects(
        ...
        if group_by_category:
            ...
            return [
                self._create_group_dict(group) for group in self._group_by_proximity(detections)
            ]
    ...
    def _group_cross_category_proximity(self, detections: list[dict]) -> list[list[dict]]:
        ...
        detections_with_category = [
            (det, self.get_semantic_category(det.get("class_name", "object"))) for det in detections
        ]
        ...
        for i, (obj1, cat1) in enumerate(detections_with_category):
            ...
            if len(group) > 1:
                categories = {
                    self.get_semantic_category(obj.get("class_name", "object")) for obj in group
                }
    ...
    def _create_group_dict(self, objects: list[dict], category: str | None = None) -> dict:
        ...
        return {
            ...
            "category": category
            or self.get_semantic_category(representative.get("class_name", "object")),
            "is_grouped": True,
        }

    def create_grouped_description(
        self, grouped_detections: list[dict], verbosity: str = "normal"
    ) -> str:
        ...
        if self.use_confidence_weighting:
            sorted_groups = sorted(
                grouped_detections, key=lambda g: g.get("confidence", 0.5), reverse=True
            )
        else:
            ...
            if count > 1:
                if verbosity == "brief":
                    ...
                    if category == "mixed":
                        ...
                        categories = {
                            self.get_semantic_category(obj.get("class_name", "object"))
...
def group_detections_semantically(
    detections: list[dict], group_by_category: bool = True, group_by_proximity: bool = True
) -> list[dict]:
...
def visualize_semantic_groups(
    image: Any, grouped_detections: list[dict], save_path: str | None = None, show: bool = False
) -> None:
    ...
    for group in grouped_detections:
        ...
        rect = patches.Rectangle(
            (x, y), width, height, linewidth=2, edgecolor=color, facecolor="none", alpha=0.7
        )
        ax.add_patch(rect)
