...
class SpatialMemory:
    ...
    def __init__(
        ...
        self.spatial_index: dict[str, Any | None] = {}  # KDTree per class.
        self._spatial_index_dirty: dict[str, bool] = defaultdict(lambda: True)

        # Thread safety.
        self._lock = Lock()

    def update(self, detections: list[dict], timestamp: float | None = None) -> None:
        ...
        with self._lock:
            ...
            for det in detections:
                ...
                self.position_history[class_name] = [
                    p for p in self.position_history[class_name] if p[2] > cutoff_time
                ]
    ...
    def _find_matching_object(
        self, class_name: str, position: tuple[float, float]
    ) -> SpatialObject | None:
            ...
            if tree is not None:
                try:
                    distances, indices = tree.query(
                        [position], k=1, distance_upper_bound=self.position_threshold
                    )
                    if distances[0] < self.position_threshold and indices[0] < len(
    ...
    def _calculate_stability(self, class_name: str, current_position: tuple[float, float]) -> float:
        ...
        variance = sum(((p[0] - cx_mean) ** 2 + (p[1] - cy_mean) ** 2) for p in positions) / len(
            positions
        )
        ...
        with self._lock:
            ...
            for objects_list in self.objects.values():
                for obj in objects_list:
                    if (
                        obj.stability >= self.stability_threshold and obj.seen_count >= 3
    ...
    def get_contextual_reminder(self, current_detections: list[dict]) -> str | None:
