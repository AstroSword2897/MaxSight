...
class AdaptiveAssistance:
    ...
    def __init__(
        ...
        self.ewma_metrics: PerformanceMetrics | None = None
        ...
        if self.use_ewma:
            if self.ewma_metrics is None:
                ...
                self.ewma_metrics = PerformanceMetrics(
                    accuracy=self.ewma_alpha * metrics.accuracy
                    + (1 - self.ewma_alpha) * self.ewma_metrics.accuracy,
                    reaction_time=self.ewma_alpha * metrics.reaction_time
                    + (1 - self.ewma_alpha) * self.ewma_metrics.reaction_time,
                    skill_progression=self.ewma_alpha * metrics.skill_progression
                    + (1 - self.ewma_alpha) * self.ewma_metrics.skill_progression,
                    hazard_awareness=self.ewma_alpha * metrics.hazard_awareness
                    + (1 - self.ewma_alpha) * self.ewma_metrics.hazard_awareness,
    ...
    def get_adaptive_verbosity(self, use_numeric: bool = False) -> Any:
        ...
        if not self.performance_history:
            if use_numeric:
                return {"brief": 0, "normal": 1, "detailed": 2, "very_detailed": 3}.get(
                    self.initial_verbosity, 1
                )
        ...
        if (
            metrics.accuracy > self.accuracy_threshold_high
            and metrics.skill_progression > self.skill_progression_threshold
        ):
        ...
        if (
            metrics.accuracy < self.accuracy_threshold_low
            or metrics.skill_progression < -self.skill_progression_threshold
        ):
    ...
    def get_adaptive_frequency(self) -> str:
        ...
        if (
            metrics.accuracy > self.accuracy_threshold_high
            and metrics.reaction_time < self.reaction_time_threshold_fast
        ):
        ...
        if (
            metrics.accuracy < self.accuracy_threshold_low
            or metrics.reaction_time > self.reaction_time_threshold_slow
        ):
...
def create_adaptive_assistance_from_session(
    session_manager, initial_verbosity: str = "detailed"
) -> AdaptiveAssistance:
    ...
    if session_manager.current_session and session_manager.task_attempts:
        ...
        if recent_tasks:
            ...
            if len(recent_tasks) >= 5:
                recent_accuracy = (
                    sum(1 for t in recent_tasks[-5:] if t.get("result", {}).get("success", False))
                    / 5
                )
                older_accuracy = (
                    sum(1 for t in recent_tasks[:5] if t.get("result", {}).get("success", False))
