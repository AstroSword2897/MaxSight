...
EDGE_CASE_SCENARIOS = [
    ...
    EdgeCaseScenario(
        name="focus_blur", description="Out-of-focus images", severity="medium", expected_drop=0.18
    ),
    # Occlusion.
    EdgeCaseScenario(
    ...
    EdgeCaseScenario(
        name="sensor_noise", description="High ISO noise", severity="medium", expected_drop=0.15
    ),
    EdgeCaseScenario(
...
def generate_edge_case_transforms() -> dict[str, Callable]:
    ...
    def rain_simulation(x: torch.Tensor) -> torch.Tensor:
        ...
        return (
            (blurred * 0.8).clamp(0, 1).squeeze(0) if x.dim() == 4 else (blurred * 0.8).clamp(0, 1)
        )
    ...
    def camera_tilt(x: torch.Tensor) -> torch.Tensor:
        ...
        angle_deg = (
            (torch.rand(1).item() * 10 - 5)
            if x.device.type != "cpu"
            else (np.random.uniform(-5, 5))
        )
...
class StressTestEvaluator:
    ...
    def __init__(self, model: nn.Module, baseline_accuracy: float = 0.0, device: str = "cpu"):
        self.model = model
    ...
    def evaluate_scenario(
        self, scenario: EdgeCaseScenario, dataloader: torch.utils.data.DataLoader
    ) -> dict:
    ...
    def run_full_stress_test(self, dataloader: torch.utils.data.DataLoader) -> dict:
        ...
        for scenario in EDGE_CASE_SCENARIOS:
            if scenario.name in self.edge_transforms:
                ...
                logger.info(
                    f"  {status} Accuracy: {result['accuracy']:.3f}, "
        ...
        critical_failures = [
            r for r in all_results if r["severity"] == "critical" and not r["within_tolerance"]
        ]
...
class InferenceBenchmarker:
    ...
    def benchmark_batch_sizes(
        ...
        for bs in batch_sizes:
            try:
                result = self.benchmark(input_shape=(bs, input_channels, input_size, input_size))
                results.append(result)
                logger.info(
                    f"Batch {bs}: {result.avg_latency_ms:.2f}ms, {result.throughput_fps:.1f} FPS"
                )
...
class PredictionFallbackSystem:
    ...
    def __init__(
        ...
        backup_model: nn.Module | None = None,
    ):
        self.model = model
    ...
    def predict_with_fallback(self, image: torch.Tensor, return_confidence: bool = True) -> dict:
...
def generate_stress_test_report(
    ...
    report = {
        ...
        "summary": {
            ...
            "avg_latency_ms": np.mean([b.avg_latency_ms for b in benchmark_results])
            if benchmark_results
            else 0,
            "peak_throughput_fps": max([b.throughput_fps for b in benchmark_results])
            if benchmark_results
            else 0,
    ...
    if stress_results.get("robustness_score", 0) < 70:
        report["recommendations"].append("Consider adding more augmentation for edge cases")
    if fallback_stats.get("fallback_rate", 0) > 0.1:
        report["recommendations"].append("High fallback rate - model may need more training data")
    if benchmark_results and benchmark_results[0].avg_latency_ms > 100:
        report["recommendations"].append("Consider model optimization for real-time performance")
