...
def apply_clahe_tensor_fast(image: torch.Tensor, clip_limit: float = 2.0) -> torch.Tensor:
...
class ImagePreprocessor:
    ...
    def __init__(self, image_size: tuple[int, int] = (224, 224), condition_mode: str | None = None):
        """Initialize image preprocessor."""
        self.image_size = image_size
        self.condition_mode = condition_mode
        # Pre-compute sharpening kernel for edge enhancement (lazy init)
        self.sharpen_kernel: torch.Tensor | None = None
        ...
        self.base_transform = transforms.Compose(
            [transforms.Resize(image_size), transforms.ToTensor(), self.normalize]
        )
    ...
    def _low_light_enhancement(self, image: Image.Image) -> Image.Image:
        ...
        img_array = (
            np.power(img_array / 255.0, gamma) * 255.0
        ...
        img_array = (
            (img_array - img_array.min()) / (img_array.max() - img_array.min() + 1e-8) * 255.0
        )
    ...
    def _analyze_lighting_condition(self, image: Image.Image) -> str:
        ...
        if len(img_array.shape) == 3:
            ...
            gray_image = np.mean(
                img_array, axis=2, dtype=np.float64
    ...
    def _simulate_bright_lighting(
        self, image: Image.Image, brightness_factor: float = 1.5
    ) -> Image.Image:
    ...
    def _simulate_dim_lighting(
        self, image: Image.Image, brightness_factor: float = 0.6
    ) -> Image.Image:
    ...
    def _simulate_dark_lighting(
        self, image: Image.Image, brightness_factor: float = 0.3
    ) -> Image.Image:
    ...
    def preprocess_with_lighting(self, image: Image.Image) -> dict[str, Any]:
        ...
        return {"image": tensor, "lighting": lighting}
    ...
    def _simulate_refractive_error(self, image: Image.Image) -> Image.Image:
        ...
        blurred = TF.gaussian_blur(
            img_tensor, kernel_size=[kernel_size, kernel_size], sigma=[sigma, sigma]
        )
    ...
    def _create_radial_mask(
        self, img_tensor: torch.Tensor, boost_center: bool = True, strength: float = 0.8
    ) -> torch.Tensor:
    ...
    def _enhance_edges(self, image: Image.Image) -> Image.Image:
        ...
        if self.sharpen_kernel is None:
            self.sharpen_kernel = (
                torch.tensor([[-1, -1, -1], [-1, 9, -1], [-1, -1, -1]], dtype=torch.float32)
                .unsqueeze(0)
                .unsqueeze(0)
            )  # [1, 1, 3, 3].
...
class DistanceEstimator:
    ...
    def detect_ground_plane(
        ...
        if not detections:
            return {"horizon_y": self.horizon_estimate, "ground_objects": [], "confidence": 0.0}
...
class TextRegionDetector:
    ...
    def detect_text_regions(
        self,
        image: np.ndarray,
        text_scores: torch.Tensor | None = None,
                ...
                for box in text_boxes:
                    if len(box) >= 4:
                        ...
                        if len(box) == 4:
                            ...
                            if (
                                w_norm * w >= self.min_text_size
                                and h_norm * h >= self.min_text_size
                            ):
        ...
        sobel_x = (
            torch.tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], device=gray.device, dtype=gray.dtype)
            .unsqueeze(0)
            .unsqueeze(0)
        )
        sobel_y = (
            torch.tensor([[-1, -2, -1], [0, 0, 0], [1, 2, 1]], device=gray.device, dtype=gray.dtype)
            .unsqueeze(0)
            .unsqueeze(0)
        )
...
def apply_batch_transforms(
    images: list[torch.Tensor], transform_fn: Callable[..., torch.Tensor], **kwargs: Any
) -> torch.Tensor:
