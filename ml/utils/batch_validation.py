...
def validate_boxes(boxes: torch.Tensor, name: str = "boxes") -> tuple[bool, str]:
    ...
    if (centers_x < 0).any() or (centers_x > 1).any():
        return (
            False,
            f"{name} has out-of-range center_x: min={centers_x.min().item()}, max={centers_x.max().item()}",
        )
    if (centers_y < 0).any() or (centers_y > 1).any():
        return (
            False,
            f"{name} has out-of-range center_y: min={centers_y.min().item()}, max={centers_y.max().item()}",
        )
...
def validate_labels(
    labels: torch.Tensor, num_classes: int, name: str = "labels"
) -> tuple[bool, str]:
    ...
    if (labels >= num_classes).any():
        return (
            False,
            f"{name} has values >= num_classes: max={labels.max().item()}, num_classes={num_classes}",
        )
...
def validate_batch(
    batch: dict[str, Any], num_classes: int = 91, check_targets: bool = True
) -> tuple[bool, str]:
...
def validate_and_sanitize_batch(
    batch: dict[str, Any], num_classes: int = 91, auto_fix: bool = True
