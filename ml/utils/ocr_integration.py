...
class OCRIntegration:
    ...
    def extract_text_from_region(
        self, image: Image.Image, region_box: list[float], use_vision_framework: bool = False
    ...
    def _extract_text_vision_framework(self, image: Image.Image) -> tuple[str | None, float]:
    ...
    def _extract_text_fallback(
        self, image: Image.Image, use_adaptive_threshold: bool = True
    ) -> tuple[str | None, float]:
        ...
        try:
            ...
            try:
                ...
                ocr_data = pytesseract.image_to_data(
                    processed_image, output_type=pytesseract.Output.DICT, config="--psm 7"
                )
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    logging.warning(
                        f"OCR failed for region {region.get('region_id', 'unknown')}: {e}"
                    )
    ...
    if verbosity == "brief":
        ...
        for i, group in enumerate(grouped_texts[:5], 1):
            pos = (
                "left"
                if group["box"][0] < 0.33
                else ("right" if group["box"][0] > 0.67 else "center")
            )
            distance = (
                "near"
                if group["box"][1] < 0.33
                else ("far" if group["box"][1] > 0.67 else "center")
            )
            descriptions.append(f"Text {i} ({pos}, {distance}): {group['text']}")
        return "; ".join(descriptions)


def _group_text_by_proximity(
    text_results: list[dict], proximity_threshold: float = 0.1
) -> list[dict]:
...
if __name__ == "__main__":
    ...
    for region in regions:
        print(
            f"  Region {region['region_id']}: confidence={region['confidence']:.2f}, box={region['box']}"
        )
