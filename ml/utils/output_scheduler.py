...
try:
    from ml.utils.sound_processing import SoundClass, SoundDirection, SoundProcessor
...
class ScheduledOutput:
    ...
    spatial_position: tuple[float, float] | None = None  # For spatial audio/haptic.
    distance: float | None = None  # Distance in meters for volume adjustment.
...
class CrossModalScheduler:
    ...
    def schedule_outputs(
        self, detections: list[dict], model_outputs: dict[str, torch.Tensor], timestamp: float
        ...
        critical_detections = [
            d for d in detections if d.get("urgency", 0) >= CRITICAL_URGENCY_THRESHOLD
        ]
        normal_detections = [
            d for d in detections if d.get("urgency", 0) < CRITICAL_URGENCY_THRESHOLD
        ]
        ...
        filtered_normal = [
            d for d in normal_detections if d.get("priority", 0) >= priority_threshold
        ]
    ...
    def _get_max_outputs(self) -> int:
        ...
        limits = {AlertFrequency.LOW: 3, AlertFrequency.MEDIUM: 5, AlertFrequency.HIGH: 10}
        return limits.get(self.config.alert_frequency, 5)

    def _create_output_for_detection(
        self, detection: dict, model_outputs: dict[str, torch.Tensor], timestamp: float
    ) -> ScheduledOutput | None:
        ...
        if spatial_pos is not None:
            ...
            distance_zone = detection.get("distance_zone")
            ...
            precise_distance = detection.get("distance_meters")
        ...
        if prev_output and prev_output.spatial_position and spatial_pos:
            ...
            if prev_output.volume_multiplier:
                ...
                if volume_diff > 0.2:  # Large change - smooth it.
                    volume_multiplier = (
                        prev_output.volume_multiplier
                        + (volume_multiplier - prev_output.volume_multiplier) * 0.6
                    )
    ...
    def _should_suppress(
        self, class_name: str, timestamp: float, priority: int, channel: OutputChannel | None = None
    ) -> bool:
    ...
    def _generate_content(self, detection: dict, model_outputs: dict) -> str:
        ...
        if box is not None:
            ...
            if urgency >= 2:
                ...
                return desc_gen.generate_hazard_alert(
                    class_name, box_tensor, distance_zone, urgency
                )
            else:
    ...
    def _create_scene_outputs(
        self, model_outputs: dict[str, torch.Tensor], detections: list[dict], timestamp: float
    ) -> list[ScheduledOutput]:
        ...
        nav_difficulty = model_outputs.get("navigation_difficulty")
        ...
        # Glare warning.
        glare_level = model_outputs.get("glare_risk_level")
...
def create_scheduler_from_profile(user_profile: dict) -> CrossModalScheduler:
    ...
    config = OutputConfig(
        ...
        haptic_intensity=user_profile.get("accessibility_preferences", {}).get(
...
class OutputValidator:
    ...
    @classmethod
    def validate_message(cls, message: str, mode: OutputMode) -> tuple[bool, str | None]:
    ...
    @classmethod
    def validate_confidence(cls, confidence: float) -> tuple[bool, str | None]:
    ...
    @classmethod
    def validate_output(cls, output: RuntimeOutput) -> tuple[bool, str | None]:
...
def create_patient_output(
    message: str, severity: Severity, confidence: float, cooldown_applied: bool = False
) -> RuntimeOutput:
