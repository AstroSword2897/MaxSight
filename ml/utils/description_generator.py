...
class DescriptionGenerator:
    ...
    def get_direction_from_box(
        self, box: torch.Tensor, image_size: tuple[int, int] = (224, 224)
    ) -> tuple[str, str]:
    ...
    def get_distance_description(self, distance_zone: int, box_size: float | None = None) -> str:
    ...
    def estimate_meters(
        self, distance_zone: int, box_size: float, object_type: str = "object"
    ) -> str | None:
    ...
    def generate_object_description(
        ...
        if verbosity == "brief":
            ...
            if urgency >= 2:
                urgency_name = (
                    self.URGENCY_NAMES[urgency] if urgency < len(self.URGENCY_NAMES) else "warning"
                )
            ...
            if urgency >= 1:
                urgency_name = (
                    self.URGENCY_NAMES[urgency] if urgency < len(self.URGENCY_NAMES) else "caution"
                )
                parts.append(f"- {urgency_name}")
    ...
    def generate_scene_description(
        self, detections: list[dict], urgency_score: int = 0, verbosity: str | None = None
    ) -> str:
        ...
        sorted_dets = sorted(
            detections, key=lambda d: (d.get("urgency", 0), d.get("priority", 0)), reverse=True
        )
        ...
        if verbosity == "brief":
            ...
            # Add scene context.
            urgency_name = (
                self.URGENCY_NAMES[urgency_score]
                if urgency_score < len(self.URGENCY_NAMES)
                else "normal"
            )
            scene_context = (
                f"Scene: {len(detections)} objects detected. Overall safety: {urgency_name}."
            )
            return scene_context + " " + ". ".join(descriptions) + "."

    def generate_navigation_guidance(
        self, detections: list[dict], target_direction: str | None = None
    ) -> str:
    ...
    def generate_hazard_alert(
        self, class_name: str, box: torch.Tensor, distance_zone: int, urgency: int
    ) -> str:
