...
def create_data_loaders(
    ...
    val_image_dir: Optional[Path] = None,
    ...
    def _resolve_image_dir(per_split: Optional[Path], shared: Optional[Path], subdir: str) -> Optional[Path]:
...
def _gold_shard_uris_from_cfg(
    ...
    if meta_path:
        ...
        if not uris:
            raise ValueError(
                f"{split_label}: gold meta at {meta_path} has no shard entries"
            )
        return uris, meta
...
def create_data_loaders_for_resolved(
    ...
    *,
    ...
    if cfg.data_plane == "gold":
        return _create_gold_data_loaders_for_resolved(
            resolved, repo_root=repo_root, device=device
        )
