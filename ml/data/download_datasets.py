...
def verify_coco_dataset(
    data_dir: Path = Path("datasets/coco"), check_coco_raw: bool = True
) -> dict[str, bool]:
...
def download_coco_dataset(data_dir: Path = Path("datasets/coco"), auto_download: bool = False):
    ...
    if auto_download:
        ...
        if shutil.which("curl"):
            download_methods.append(
                ("curl", ["curl", "-L", "-C", "-", "--progress-bar", "--retry", "3"])
            )
    ...
    if not auto_download:
        ...
        print(
            '  Run: python -c "from ml.data.download_datasets import download_coco_dataset; download_coco_dataset(auto_download=True)"'
        )
...
def create_synthetic_impairments():
    ...
    def apply_blur(image: np.ndarray, sigma: float = 3.0) -> np.ndarray:
        ...
        if len(image.shape) == 3:
            return np.stack(
                [
                    ndimage.gaussian_filter(image[:, :, i], sigma=sigma)
    ...
    def apply_peripheral_mask(image: np.ndarray, mask_radius: float = 0.7) -> np.ndarray:
        ...
        y, x = np.ogrid[:h, :w]
        mask = (
            np.sqrt((x - center_x) ** 2 + (y - center_y) ** 2) / max(center_x, center_y)
            < mask_radius
        )
        if len(image.shape) == 3:
    ...
    def apply_color_shift(image: np.ndarray, shift_type: str = "protanopia") -> np.ndarray:
        ...
        if shift_type == "protanopia":
            ...
            matrix = np.array([[0.625, 0.375, 0.0], [0.7, 0.3, 0.0], [0.0, 0.3, 0.7]])
        else:  # Tritanopia.
            # Blue-yellow color blindness.
            matrix = np.array([[0.95, 0.05, 0.0], [0.0, 0.433, 0.567], [0.0, 0.475, 0.525]])
