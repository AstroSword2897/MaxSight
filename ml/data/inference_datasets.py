...
class ADE20KDataset(Dataset):
    ...
    def _load_image_list(self) -> list[dict[str, Any]]:
        ...
        for img_file in image_dir.glob("*.jpg"):
            ...
            annotation_path = (
                annotation_dir / f"{image_id}.png" if annotation_dir.exists() else None
            )
...
class DetectionPostProcessor:
    ...
    def process(
        self, model: torch.nn.Module, outputs: dict[str, torch.Tensor], batch_size: int
            ...
            try:
                ...
                if isinstance(detections, list):
                    ...
                    if len(detections) != batch_size:
                        logger.warning(
                            f"Detections length {len(detections)} != batch_size {batch_size}"
                        )
                        # Pad or truncate.
                        if len(detections) < batch_size:
                    ...
                    return [
                        detections[i : i + self.max_detections]
                        for i in range(0, len(detections), self.max_detections)
                    ]
            except Exception as e:
                logger.warning(
                    f"model.get_detections failed: {e}, falling back to manual processing"
                )
                ...
                if valid_mask.sum() > 0:
                    ...
                    for idx in top_indices:
                        batch_detections.append(
                            {
                                "box": valid_boxes[idx].cpu().tolist()
                                if torch.is_tensor(valid_boxes[idx])
                                else valid_boxes[idx],
                                "confidence": valid_scores[idx].item()
                                if torch.is_tensor(valid_scores[idx])
                                else valid_scores[idx],
...
def create_inference_dataloader(
    ...
    max_samples: int | None = None,
    shuffle: bool = False,
    pin_memory: bool | None = None,  # FIXED: Make configurable.
) -> DataLoader:
    """Create DataLoader for inference datasets."""
    if dataset_name.lower() == "open_images_v6":
        dataset = OpenImagesV6Dataset(root=root, split=split, max_samples=max_samples)
    elif dataset_name.lower() == "bdd100k":
        dataset = BDD100KDataset(root=root, split=split, max_samples=max_samples)
    elif dataset_name.lower() == "ade20k":
        dataset = ADE20KDataset(root=root, split=split, max_samples=max_samples)
    else:
        raise ValueError(
            f"Unknown dataset: {dataset_name}. Choose from: open_images_v6, bdd100k, ade20k"
        )
...
def run_inference_on_dataset(
    ...
    if postprocessor is None:
        postprocessor = DetectionPostProcessor(confidence_threshold=0.3, max_detections=10)
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            for i in range(batch_size):
                ...
                pred = {
                    ...
                    if "image_id" in batch and i < len(batch["image_id"])
                    else f"img_{global_image_idx}",
                    "dataset": batch["dataset"][i]
                    if "dataset" in batch and i < len(batch["dataset"])
                    else "unknown",
                    "context": {
                        ...
                        "annotation_path": (context.get("annotation_path") or "")
                        if isinstance(context, dict)
                        else "",
    ...
    stats = {
        ...
        "avg_detections_per_image": sum(p["num_detections"] for p in all_predictions) / total
        if total > 0
        else 0,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        choices=["open_images_v6", "bdd100k", "ade20k"],
        required=True,
        help="Dataset to use",
    )
    parser.add_argument("--root", type=Path, required=True, help="Root directory for dataset")
    parser.add_argument(
        "--split", type=str, default="validation", help="Dataset split (default: validation)"
    )
    parser.add_argument("--batch_size", type=int, default=32, help="Batch size (default: 32)")
    parser.add_argument(
        "--model_path", type=Path, default=None, help="Path to trained model checkpoint"
    )
    parser.add_argument(
        "--device", type=str, default="cpu", help="Device to run inference on (default: cpu)"
    )
    parser.add_argument(
        "--max_samples", type=int, default=None, help="Number of samples to process (default: all)"
    )
    parser.add_argument(
        "--skip_corrupted",
        action="store_true",
