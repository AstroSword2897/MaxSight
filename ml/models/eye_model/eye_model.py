...
class EyeImagePreprocessor:
    ...
    def __call__(self, image: Image.Image) -> torch.Tensor:
        ...
        if not isinstance(result, torch.Tensor):
            ...
            result = TF.resize(result.unsqueeze(0), list(self.target_size)).squeeze(
                0
            )  # Convert tuple to list.
        return result

    def preprocess_tensor(
        self, tensor: torch.Tensor, ensure_normalized: bool = True
    ) -> torch.Tensor:
...
class EyeModel(nn.Module):
    ...
    def __init__(self, input_size: tuple[int, int] = (64, 64), dropout: float = 0.15):
    ...
    def forward(self, face_region: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        # Validate outputs.
        if (
            torch.isnan(blink_prob).any()
            or torch.isnan(fixation).any()
            or torch.isnan(pupil_size).any()
        ):
            raise RuntimeError(
