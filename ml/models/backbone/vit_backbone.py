...
class VisionTransformerBackbone(nn.Module):
    ...
    def __init__(
        ...
        if use_learned_pos:
            ...
            pos_embed = create_sinusoidal_pos_embedding(self.num_patches + 1, embed_dim)
    ...
    def forward(
        self, x: torch.Tensor, return_patch_tokens: bool = True
    ...
    def get_intermediate_layers(self, x: torch.Tensor, n: int = 4) -> list:
...
class EfficientCrossModalAttention(nn.Module):
    ...
    def __init__(self, dim: int, num_heads: int = 8, dropout: float = 0.1):
        ...
        if self.use_flash:
            ...
            self.cnn_to_vit = nn.MultiheadAttention(
                embed_dim=dim, num_heads=num_heads, dropout=dropout, batch_first=True, bias=False
            )
    ...
    def forward(
        ...
        if self.use_flash:
            ...
            # CNN queries ViT.
            qkv_cnn = self.qkv_cnn(cnn_feat).reshape(
                B, -1, 3, self.num_heads, self.dim // self.num_heads
            )
            q_cnn, _, _ = qkv_cnn.unbind(2)

            qkv_vit = self.qkv_vit(vit_feat).reshape(
                B, -1, 3, self.num_heads, self.dim // self.num_heads
            )
...
class FeatureCache:
    ...
    def _make_cache_key(self, x: torch.Tensor, frame_id: int | None = None) -> str:
...
class HybridCNNViTBackbone(nn.Module):
    ...
    def __init__(
        ...
        cross_layer_alpha: float | None = None,  # Now learnable if None.
        ...
        self.cnn_stem = nn.Sequential(resnet.conv1, resnet.bn1, resnet.relu, resnet.maxpool)
        ...
        self.fpn = FeaturePyramidNetwork(
            in_channels_list=self.fpn_in_channels, out_channels=cnn_out_channels
        )
        ...
        try:
            ...
            if hasattr(current_module, "VisionTransformerBackbone"):
                VisionTransformerBackbone = current_module.VisionTransformerBackbone
            else:
        ...
        if use_cross_layer_connections:
            ...
            if use_bidirectional_attention:
                self.cross_modal_attn = EfficientCrossModalAttention(
                    dim=min(cnn_out_channels, vit_embed_dim), num_heads=8, dropout=dropout
                )
                # Alignment projections.
                if cnn_out_channels != vit_embed_dim:
                    self.cnn_align = nn.Linear(
                        cnn_out_channels * len(fpn_levels),
                        min(cnn_out_channels, vit_embed_dim),
                        bias=False,
                    )
                    self.vit_align = nn.Linear(
                        vit_embed_dim, min(cnn_out_channels, vit_embed_dim), bias=False
                    )
        ...
        if fusion_method == "concat":
            ...
            self.fusion = ImprovedAdaptiveFusion(
                cnn_dim=total_cnn_dim, vit_dim=vit_embed_dim, fused_dim=fused_dim
            )
            ...
            self.cross_attn = nn.MultiheadAttention(
                embed_dim=fused_dim, num_heads=8, dropout=dropout, batch_first=True, bias=False
            )
    ...
    def extract_cnn_features(
        self, x: torch.Tensor, frame_id: int | None = None
    ) -> list[torch.Tensor]:
    ...
    def cnn_vit_interaction(
        self, cnn_features: list[torch.Tensor], vit_patches: torch.Tensor
        ...
        for cnn_feat, proj_layer in zip(cnn_features, self.cnn_to_vit_proj):
            ...
            vit_patches = vit_patches.add_(cnn_pooled.flatten(2).transpose(1, 2).mul_(alpha))

            # ViT → CNN.
            H, W = cnn_feat.shape[2:]
            vit_resized = (
                F.interpolate(vit_projected, (H, W), mode="bilinear", align_corners=False)
                if vit_projected.shape[2:] != (H, W)
                else vit_projected
            )
    ...
    def forward(
        ...
        if self.use_gradient_checkpointing and self.training:
            vit_cls, vit_patches = torch.utils.checkpoint.checkpoint(
                lambda x: self.vit(x, return_patch_tokens=True), images, use_reentrant=False
            )
        else:
        ...
        if self.use_cross_layer_connections:
            ...
            if self.use_bidirectional_attention:
                cnn_global = torch.cat(
                    [F.adaptive_avg_pool2d(f, 1).flatten(1) for f in fpn_features], dim=1
                )
