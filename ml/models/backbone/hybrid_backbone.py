...
class AdaptiveFeatureFusion(nn.Module):
    ...
    def __init__(self, cnn_dim: int, vit_dim: int, fused_dim: int, num_heads: int = 8):
        ...
        self.cnn_proj = nn.Sequential(
            nn.Linear(cnn_dim, fused_dim), nn.LayerNorm(fused_dim), nn.GELU()
        )
        self.vit_proj = nn.Sequential(
            nn.Linear(vit_dim, fused_dim), nn.LayerNorm(fused_dim), nn.GELU()
        )
        ...
        self.output_proj = nn.Sequential(nn.Linear(fused_dim, fused_dim), nn.LayerNorm(fused_dim))
...
class CrossModalAttention(nn.Module):
    ...
    def forward(
        ...
        # CNN queries ViT.
        cnn_enhanced, _ = self.cnn_to_vit(query=cnn_feat, key=vit_feat, value=vit_feat)
        cnn_feat = self.norm1(cnn_feat + self.dropout(cnn_enhanced))

        # ViT queries CNN.
        vit_enhanced, _ = self.vit_to_cnn(query=vit_feat, key=cnn_feat, value=cnn_feat)
...
class HybridCNNViTBackbone(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(cross_layer_alpha, float):
            ...
            self.cross_layer_alpha_raw = (
                cross_layer_alpha
                if cross_layer_alpha is not None
                else nn.Parameter(torch.tensor(0.1))
            )
        ...
        self.fpn = FeaturePyramidNetwork(
            in_channels_list=[512, 1024, 2048], out_channels=cnn_out_channels
        )
        ...
        if fusion_method == "concat":
            ...
            self.cross_attn = nn.MultiheadAttention(
                embed_dim=fused_dim, num_heads=8, dropout=dropout, batch_first=True
            )
    ...
    def extract_cnn_features(
        ...
        if self.use_gradient_checkpointing and self.training:
            c3, c4, c5 = torch.utils.checkpoint.checkpoint(run_cnn, use_reentrant=False)
        else:
            c3, c4, c5 = run_cnn()
    ...
    def cnn_vit_interaction(
        ...
        vit_spatial = vit_patches.transpose(1, 2).reshape(B, self.vit_embed_dim, patch_h, patch_w)

        enhanced_cnn = []

        for i, (cnn_feat, cnn_to_vit_layer) in enumerate(zip(cnn_features, self.cnn_to_vit_proj)):
            ...
            cnn_to_vit_resized = F.adaptive_avg_pool2d(cnn_projected, (patch_h, patch_w))
            ...
            if hasattr(self, "cross_layer_alpha_raw"):
                ...
                alpha = torch.sigmoid(
                    ...
                    if isinstance(raw, torch.Tensor)
                        ...
                        float(raw), device=vit_patches.device, dtype=vit_patches.dtype
                    )
                )
            else:
            ...
            vit_to_cnn_resized = F.interpolate(
                vit_projected, size=cnn_feat.shape[2:], mode="bilinear", align_corners=False
            )
            ...
            alpha = torch.sigmoid(
                raw
                if isinstance(raw, torch.Tensor)
                else torch.tensor(float(raw), device=cnn_feat.device, dtype=cnn_feat.dtype)
            )
    ...
    def forward(
        self,
        x: torch.Tensor,
        vit_patches: torch.Tensor | None = None,
        ...
        if vit_patches is None:
            if self.use_gradient_checkpointing and self.training:
                vit_cls, vit_patches = torch.utils.checkpoint.checkpoint(
                    lambda img: self.vit(img, return_patch_tokens=True), x, use_reentrant=False
                )
            else:
