...
class SceneGraphEncoder(nn.Module):
    def __init__(
        ...
        self.spatial_classifier = nn.Sequential(
            nn.Linear(object_embed_dim * 2, 256), nn.ReLU(), nn.Linear(256, num_spatial_relations)
        )

        # Trainable semantic classifier.
        self.semantic_classifier = nn.Sequential(
            nn.Linear(object_embed_dim * 2, 256), nn.ReLU(), nn.Linear(256, num_semantic_relations)
        )

        # Relation embeddings.
        self.relation_embedding = nn.Embedding(
            num_spatial_relations + num_semantic_relations, relation_embed_dim
        )
    ...
    def extract_semantic_relations(
        self, object_classes: list[str], object_embeddings: torch.Tensor
    ) -> list[SceneRelation]:
    ...
    def extract_relations(
        ...
        if len(all_relations) > 0:
            ...
            for rel in all_relations:
                ...
                if rel in spatial_relations:
                    ...
                    pred_idx = (
                        ...
                        + hash(rel.predicate) % self.num_semantic_relations
                    )
                relation_indices.append(pred_idx)

            relation_indices_tensor = torch.tensor(
                relation_indices, dtype=torch.long, device=boxes.device
            )
            ...
            edge_attr = torch.empty(
                (0, self.relation_embed_dim), dtype=torch.float32, device=boxes.device
            )

        return all_relations, edge_index, edge_attr

    def forward(
        self, boxes: torch.Tensor, object_embeddings: torch.Tensor, object_classes: list[str]
    ) -> dict[str, object]:
        if boxes.dim() == 3:
            ...
            edge_attr = torch.empty(
                (0, self.relation_embed_dim), dtype=torch.float32, device=object_embeddings.device
            )
            batch = torch.arange(batch_size, device=object_embeddings.device).repeat_interleave(
                boxes.shape[1]
            )
