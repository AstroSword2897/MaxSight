    ...
    def __init__(
        ...
        if haptic_dim > 0:
            self.output_proj = nn.Sequential(
                nn.Linear(embed_dim * 3, embed_dim), nn.Dropout(dropout)
            )
        else:
            self.output_proj = nn.Sequential(
...
class CrossTaskAttention(nn.Module):
    ...
    def forward(
        ...
        desc_enhanced, _ = self.detection_to_description(
            query=desc_proj, key=det_enhanced, value=det_enhanced
        )
        ...
        if desc_enhanced.shape[1] == 1:
            ...
            desc_expanded = (
                desc_enhanced[:, :N_text, :]
                if desc_enhanced.shape[1] >= N_text
                else desc_enhanced.repeat(1, (N_text // desc_enhanced.shape[1]) + 1, 1)[
                    :, :N_text, :
                ]
            )

        ocr_enhanced, _ = self.description_to_ocr(
            query=ocr_proj, key=desc_expanded, value=desc_expanded
        )
