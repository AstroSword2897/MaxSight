...
class MultiVectorRetrievalHeads(nn.Module):
    ...
    def forward(
        ...
        audio: torch.Tensor | None = None,  # [B, audio_features].
        text_snippets: list[list[str]] | None = None,  # Variable length per image.
        ...
        if audio is not None:
            audio_emb, spatial = self.audio_encoder(
                audio
            )  # Audio_emb: [B, audio_dim] or [B, A, audio_dim].
            if audio_emb.dim() == 2:
