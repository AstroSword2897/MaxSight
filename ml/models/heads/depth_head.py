...
class DepthHead(nn.Module):
    ...
    def forward(
        ...
        motion_features: torch.Tensor | None = None,  # FIXED: Motion as temporal anchor.
        ...
        if motion_features is not None and hasattr(self, "motion_proj"):
            ...
            if motion_proj.shape[2:] != features.shape[2:]:
                motion_proj = F.interpolate(
                    motion_proj, size=features.shape[2:], mode="bilinear", align_corners=False
                )
        ...
        if self.use_multi_scale and fpn_features is not None:
            ...
            if len(depth_feat_list) > 1:
                depth_feat = torch.stack(depth_feat_list, dim=0).mean(
                    dim=0
        ...
        depth_flat = (
            depth_map.contiguous().reshape(B, -1).float()
        )  # [B, H*W] - ensure float for quantile.
