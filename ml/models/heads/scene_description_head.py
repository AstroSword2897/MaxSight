...
class SceneDescriptionHead(nn.Module):
    ...
    def __init__(
        ...
        self.verbosity_controller = nn.ModuleDict(
            {mode: nn.Linear(embed_dim, embed_dim) for mode in self.condition_modes}
        )
    ...
    def _generate_autoregressive(self, memory: torch.Tensor, device: torch.device) -> torch.Tensor:
    ...
    def forward(
        ...
        ocr_embeddings: torch.Tensor | None = None,  # [B, N_text, ocr_dim].
        condition_mode: str = "normal",
        target_text: torch.Tensor | None = None,  # [B, seq_len] for training.
        roi_priorities: torch.Tensor | None = None,  # [B, N_regions] - FIXED: ROI priority weights.
        ...
        return {"description_logits": description_logits, "description": decoded}
