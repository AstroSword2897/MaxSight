...
class ROIPriorityHead(nn.Module):
    ...
    def __init__(
        ...
        if use_attention:
            self.attention = nn.MultiheadAttention(
                embed_dim=roi_dim, num_heads=num_heads, dropout=dropout, batch_first=True
            )
    ...
    def forward(
        ...
        if scene_embedding.dim() != 2:
            raise ValueError(
                f"Expected 2D scene_embedding [B, scene_dim], got {scene_embedding.shape}"
            )
    ...
    def compute_ranking_loss(
        self, scores: torch.Tensor, rankings: torch.Tensor, margin: float = 0.1
    ) -> torch.Tensor:
