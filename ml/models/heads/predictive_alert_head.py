...
class PredictiveAlertHead(nn.Module):
    ...
    def forward(
        ...
        motion_features: torch.Tensor | None = None,  # [B, motion_dim].
        spatial_memory: torch.Tensor | None = None,  # [B, N_objects, 4] (boxes)
        ...
        combined = torch.cat(
            [scene_features, motion_features], dim=1
        )  # [B, input_dim + motion_dim].
        ...
        action_input = torch.cat(
            [combined, hazard_probs], dim=1
        )  # [B, input_dim + motion_dim + num_hazard_types].
