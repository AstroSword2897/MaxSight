...
class FatigueHead(nn.Module):
    ...
    def forward(
        self, eye_features: torch.Tensor, motion_features: torch.Tensor
        ...
        if motion_features.dim() != 2:
            raise ValueError(
                f"Expected 2D motion_features [B, motion_dim], got {motion_features.shape}"
            )
        ...
        if B_eye != B_motion:
            raise ValueError(
                f"Batch size mismatch: eye_features {B_eye} vs motion_features {B_motion}"
            )
        ...
        if motion_features.shape[1] != self.temporal_dim:
            raise ValueError(
                f"Expected motion_dim={self.temporal_dim}, got {motion_features.shape[1]}"
            )
