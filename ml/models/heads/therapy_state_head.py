...
class TherapyStateHead(nn.Module):
    ...
    def __init__(
        ...
        if motion_dim > 0:
            self.motion_proj_depth = nn.Conv2d(
                motion_dim, in_channels_depth, kernel_size=1, bias=False
            )
        ...
        if use_depth_multi_scale:
            self.fpn_proj = nn.ModuleDict(
                {"p3": nn.Conv2d(256, 64, 1), "p4": nn.Conv2d(256, 64, 1)}
            )
        ...
        if motion_dim > 0:
            self.motion_proj_contrast = nn.Conv2d(
                motion_dim, in_channels_contrast, kernel_size=1, bias=False
            )
        ...
        self.conv1_contrast = nn.Conv2d(
            in_channels_contrast, 128, kernel_size=3, padding=1, bias=False
        )
    ...
    def _make_head(self, input_dim: int) -> nn.Module:
        ...
        return nn.Sequential(
            nn.Linear(input_dim, 16), nn.ReLU(inplace=True), nn.Linear(16, 1), nn.Sigmoid()
        )
    ...
    def forward(
        ...
        fpn_features: dict[str, torch.Tensor] | None = None,
        ...
        if motion_features.dim() == 4:
            ...
            raise ValueError(
                f"Expected motion_features to be 2D [B, D] or 4D [B, D, H, W], got {motion_features.shape}"
            )
        ...
        if motion_1d.shape[1] != self.temporal_dim:
            ...
            if not hasattr(self, "motion_proj_fatigue"):
                self.motion_proj_fatigue = nn.Linear(motion_1d.shape[1], self.temporal_dim).to(
                    device
                )
        ...
        if motion_features.dim() == 4 and hasattr(self, "motion_proj_depth"):
            ...
            if motion_proj.shape[2:] != depth_features.shape[2:]:
                motion_proj = F.interpolate(
                    motion_proj, size=depth_features.shape[2:], mode="bilinear", align_corners=False
                )
            ...
            for scale_name, proj in self.fpn_proj.items():
                if scale_name in fpn_features:
                    ...
                    proj_feat = F.interpolate(
                        proj_feat, size=depth_feat.shape[2:], mode="bilinear", align_corners=False
                    )
