...
class PersonalizationHead(nn.Module):
    ...
    def __init__(
        ...
        self.adaptation_gate = nn.Sequential(
            nn.Linear(embed_dim + interaction_dim, embed_dim), nn.Sigmoid()
        )
        ...
        self.fusion = nn.Sequential(nn.Linear(fused_dim, 512), nn.ReLU())
    ...
    def forward(
        ...
        interaction_features: torch.Tensor | None = None,  # [B, interaction_dim].
        ...
        return {
            ...
            "attention_weights": torch.softmax(attention_logits / self.crisp, dim=1),
            "alert_priority_weights": torch.softmax(alert_logits / self.crisp, dim=1),
