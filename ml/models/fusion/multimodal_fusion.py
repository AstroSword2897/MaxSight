...
class EnhancedAudioEncoder(nn.Module):
    ...
    def forward(
        ...
        stereo_channels: torch.Tensor | None = None,  # [B, T, 2] for directional.
...
class MultimodalFusion(nn.Module):
    ...
    def __init__(
        ...
        self.depth_token = (
            nn.Parameter(torch.randn(1, 1, embed_dim) * 0.02) if depth_dim > 0 else None
        )
        self.haptic_token = (
            nn.Parameter(torch.randn(1, 1, embed_dim) * 0.02) if haptic_dim > 0 else None
        )
    ...
    def forward(
        ...
        depth_features: torch.Tensor | None = None,  # [B, depth_dim].
        haptic_features: torch.Tensor | None = None,  # [B, haptic_dim].
    ) -> torch.Tensor:
...
class SpatialSoundMapping(nn.Module):
    ...
    def __init__(
        self,
        audio_dim: int = 256,
        attention_size: tuple[int, int] = (14, 14),
        ...
        self.attention_generator = nn.Sequential(
            nn.Linear(audio_dim + num_directions + 1, 128),
            nn.ReLU(),
            nn.Linear(128, attention_size[0] * attention_size[1]),
            nn.Sigmoid(),
        )

    def forward(
        self, audio_features: torch.Tensor, stereo_channels: torch.Tensor | None = None
    ...
    def apply_audio_attention(
        self, visual_features: torch.Tensor, attention_map: torch.Tensor
    ) -> torch.Tensor:
        if attention_map.shape[2:] != visual_features.shape[2:]:
            attention_map = F.interpolate(
                attention_map, size=visual_features.shape[2:], mode="bilinear", align_corners=False
            )
...
class HapticVisualAttention(nn.Module):
    ...
    def __init__(
        self, haptic_embed_dim: int = 128, visual_embed_dim: int = 256, attention_dim: int = 128
    ):
        super().__init__()
    ...
    def forward(
        self, haptic_embedding: torch.Tensor, visual_features: torch.Tensor
    ) -> tuple[torch.Tensor, torch.Tensor]:
        haptic_proj = self.haptic_proj(haptic_embedding).unsqueeze(1)
        visual_proj = self.visual_proj(visual_features)
        attended, attn_weights = self.attention(
            query=haptic_proj, key=visual_proj, value=visual_proj
        )
