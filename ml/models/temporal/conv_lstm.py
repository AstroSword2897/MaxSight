...
class ConvLSTM(nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dim: int, kernel_size: int = 3, num_layers: int = 2):
        super().__init__()
...
class DividedSpaceTimeAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_temporal = self.norm2(
            x.permute(0, 2, 1, 3).contiguous().reshape(B * N, T, D) + x_temporal
        )
...
class TimeSformer(nn.Module):
    ...
    def __init__(
        self, embed_dim: int = 768, num_heads: int = 12, num_layers: int = 12, num_frames: int = 8
    ):
        super().__init__()
        ...
        self.blocks = nn.ModuleList(
            [DividedSpaceTimeAttention(embed_dim, num_heads) for _ in range(num_layers)]
        )
