...
class TemporalEncoder(nn.Module):
    ...
    def __init__(
        ...
        if use_conv_lstm:
            self.conv_lstm = ConvLSTM(
                input_dim=in_channels, hidden_dim=hidden_dim, kernel_size=3, num_layers=2
            )
        ...
        if use_timesformer:
            if TimeSformer is None:
                raise ImportError(
                    "TimeSformer module not found. Set use_timesformer=False or install temporal_transformer module."
                )
            self.timesformer = TimeSformer(
                embed_dim=vit_embed_dim, num_heads=12, num_layers=12, num_frames=num_frames
            )
    ...
    def forward(
        ...
        vit_patch_tokens: torch.Tensor | None = None,
        ...
        if self.use_conv_lstm:
            ...
            if motion.shape[-1] > 56:  # Only downsample if resolution is high.
                motion = F.interpolate(
                    motion, scale_factor=0.5, mode="bilinear", align_corners=False
                )
        ...
        if self.use_conv_lstm and motion_features is not None and motion_features.shape[1] >= 2:
            ...
            flicker_feat = torch.abs(
                motion_features[:, -1] - motion_features[:, -2]
            )  # [B, hidden_dim, H, W].
        else:
