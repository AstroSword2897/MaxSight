...
class MaxSightCNN(nn.Module):
    ...
    def __init__(
        ...
        if use_audio:
            ...
            self.audio_encoder = EnhancedAudioEncoder(input_dim=128, embed_dim=256, num_heads=8)
        ...
        if tier_config.use_cross_task_attention:
            ...
            self.cross_task_attention = CrossTaskAttention(
                detection_dim=256, ocr_dim=256, description_dim=512, embed_dim=512, num_heads=8
            )
        else:
        ...
        try:
            self.global_encoder = GlobalEncoder(embed_dim=512, use_clip=True)
            ...
            self.global_encoder = GlobalEncoder(embed_dim=512, use_clip=False)

        # Retrieval is async and advisory; it never blocks inference.
        self.enable_retrieval = (
            tier_config.use_retrieval if hasattr(tier_config, "use_retrieval") else False
        )
        ...
        if self.enable_retrieval:
            try:
                ...
                knowledge_augment = (
                    KnowledgeAugmentedRetrieval(node_dim=256, embed_dim=512)
                    if self.enable_retrieval
                    else None
                )
        ...
        self.personalization_head = PersonalizationHead(
            input_dim=512, num_features=10, num_alert_types=5, embed_dim=256
        )
        ...
        self.object_encoder = nn.Sequential(
            nn.Linear(256, 256), nn.LayerNorm(256), nn.ReLU(), nn.Linear(256, 256)
        )
        ...
        if enable_accessibility_features:
            ...
            self.contrast_head = ContrastMapHead(
                in_channels=256, motion_dim=256, use_edge_aware=True
            )
            ...
            self.uncertainty_head = GlobalConfidenceAggregator(
                scene_dim=256, hidden_dim=128, dropout=0.1
            )
    ...
    def _forward_stage_a_backbone(
        ...
        p3_resized = F.interpolate(
            p3, size=p4.shape[2:], mode="bilinear", align_corners=False
        ).contiguous()
        p5_resized = F.interpolate(
        ...
        fused_features = (
            fused_features.contiguous()
        )  # CUDA/cpu: contiguous after fusion for downstream.
    ...
    def _forward_stage_b_backbone(
        ...
        if (
            self.tier_config.use_temporal_modeling
            and temporal_mode
            and B_orig is not None
            and T is not None
            ...
            if motion_features is not None:
                ...
                if motion_features.dim() == 4:  # [B, C, H, W]
                    ...
                    motion_features = motion_features.contiguous().reshape(
                        B_orig * T, -1, H_temp, W_temp
                    )
                ...
                if (
                    ...
                    and motion_features.shape[2:] != stage_b_features.shape[2:]
                ):
    ...
    def forward(
        ...
        enable_scene_graph = (
            self.tier_config.use_cross_task_attention
            if (hasattr(self, "tier_config") and self.tier_config is not None)
            else False
        )
        ...
        if audio_features is not None and self.use_audio and self.audio_encoder is not None:
            ...
            if self.sound_event_head is not None:
                sound_outputs = self.sound_event_head(
            ...
            if audio_attention_map is not None:
                ...
                audio_attention_map = torch.sigmoid(
                    audio_attention_map
                )  # [0, 1] with smooth gradients.
                fused_features = fused_features * (
                    1.0 + audio_attention_map
        ...
        condition_blur = self.condition_mode in [
            "cataracts",
            "refractive_errors",
            "myopia",
            "hyperopia",
        ...
        if condition_inconsistent and hasattr(self, "attention_weights"):
            ...
            fused_features = (
                attn[1] * p3_resized + attn[2] * p4 + attn[3] * p5_resized
            ) * 0.5 + fused_features * 0.5
        ...
        cls_logits = (
            cls_logits.permute(0, 2, 3, 1).contiguous().reshape(batch_size, H * W, self.num_classes)
        )
        ...
        default_box = torch.tensor(
            [0.5, 0.5, 0.1, 0.1], device=box_preds.device, dtype=box_preds.dtype
        )
        box_preds = torch.where(nan_inf_mask, default_box.unsqueeze(0).unsqueeze(0), box_preds)
        obj_logits = torch.where(
            torch.isnan(obj_logits) | torch.isinf(obj_logits),
            torch.zeros_like(obj_logits),
            obj_logits,
        )
        text_logits = torch.where(
            torch.isnan(text_logits) | torch.isinf(text_logits),
            torch.zeros_like(text_logits),
            text_logits,
        )
        cls_logits = torch.where(
            torch.isnan(cls_logits) | torch.isinf(cls_logits),
        ...
        obj_scores = torch.sigmoid(
            obj_logits
        )  # Objectness confidence - probability there's an object.
        ...
        distances_flat = self.distance_head(
            dist_input.contiguous().reshape(-1, dist_input.size(-1))
        )
        distances = distances_flat.contiguous().reshape(
            batch_size, H * W, self.num_distance_zones
        )  # [B, H*W, 3].

        depth_outputs = self.depth_head_module(fused_features, motion_features=None)
        depth_map = depth_outputs["depth_map"]  # [B, H, W].
        depth_uncertainty = depth_outputs["uncertainty"]  # [B, H, W].

        top_k_depth = min(10, int(H) * int(W))
        top_k_scores_depth, top_k_indices_depth = torch.topk(
            obj_scores, k=top_k_depth, dim=1
        )  # [B, K].

        # Extract box centers for top-K.
        box_centers = box_preds[:, :, :2]  # [B, H*W, 2] - x, y centers.
        top_k_centers = torch.gather(
            box_centers, dim=1, index=top_k_indices_depth.unsqueeze(-1).expand(-1, -1, 2)
        )  # [B, K, 2].

        cache_key = (int(H), int(W))
        if not hasattr(self, "_grid_norm_cache") or self._grid_norm_cache.get("key") != cache_key:
            self._grid_norm_cache = {
                "key": cache_key,
                "scale": torch.tensor(
                    [float(W), float(H)], device=images.device, dtype=torch.float32
                ),
            }
        scale = self._grid_norm_cache["scale"].to(images.device)
        normalized_centers = (top_k_centers / scale.unsqueeze(0).unsqueeze(0)) * 2.0 - 1.0
        normalized_centers = normalized_centers.flip(-1).unsqueeze(
            2
        )  # [B, K, 1, 2] for grid_sample.
        ...
        top_k_classes_depth = torch.gather(
            cls_logits.argmax(dim=-1), dim=1, index=top_k_indices_depth
        )  # [B, K].
        ...
        stage_a_outputs = {
            ...
            "num_locations": _scalar_tensor_num_locations(
                H, W, images.device
            ),  # Scalar tensor for JIT trace (no int in dict)
        }
        ...
        skip_stage_b = (
            (uncertainty_score is not None and (uncertainty_score > 0.7).any())
        ...
        if stage_a_start_time is not None:
            ...
            if stage_a_latency_ms > max_latency:
                ...
                if hasattr(self, "_timing_warnings"):
                    ...
                    self._timing_warnings.append(
                        f"Stage A latency {stage_a_latency_ms:.2f}ms exceeds threshold, skipping Stage B"
                    )
        ...
        outputs["motion"] = (
            motion_features
            if motion_features is not None
            else torch.zeros(batch_size, 2, 1, 1, device=dev, dtype=fused_features.dtype)
        )
        if c is not None:
        ...
        if sound_outputs is not None:
            ...
            if enable_cross_task_heads:
                ...
                assert combined_context.is_contiguous(), (
                    "combined_context not contiguous before shared_scene_embedding"
                )
                ...
                contrast_outputs = self.contrast_head(
                    det_feats
                )  # Dict with contrast_map, edge_map.
                contrast_map = contrast_outputs.get(
                    "contrast_map", contrast_outputs.get("contrast_sensitivity")
                )  # [B, 1, H, W].
                ...
                temporal_features = torch.zeros(
                    batch_size, 128, device=det_feats.device
                )  # From temporal encoder if available.
                fatigue_outputs = self.fatigue_head(
                    eye_features, temporal_features
                )  # Dict with scores.
                ...
                roi_features = (
                    det_feats.permute(0, 2, 3, 1).contiguous().view(batch_size, H * W, -1)
                )  # [B, H*W, 256].
                ...
                alert_outputs = self.predictive_alert_head(
                    ...
                    motion_features=motion_magnitude.mean(dim=[2, 3])
                ...
                findability_scores = (
                    findability_scores.permute(0, 2, 3, 1).contiguous().reshape(batch_size, H * W)
                )  # [B, H*W].
                ...
                outputs.update(
                    {
                        ...
                        "contrast_sensitivity": contrast_map.mean(dim=[2, 3])
                        if contrast_map.dim() == 4
                        else contrast_map,
        ...
        batch_indices = (
            torch.arange(batch_size, device=det_feats.device).unsqueeze(1).expand(-1, top_k_scene)
        )
        object_embeddings = det_feats[batch_indices, :, y_indices, x_indices]

        top_k_boxes = torch.gather(
            box_preds, dim=1, index=top_k_indices_scene.unsqueeze(-1).expand(-1, -1, 4)
        )  # [B, K, 4].

        top_k_classes_scene = torch.gather(
            cls_logits.argmax(dim=-1), dim=1, index=top_k_indices_scene
        )  # [B, K].
        ...
        enable_scene_graph = (
            self.tier_config.use_cross_task_attention if hasattr(self, "tier_config") else False
        )
        ...
        if enable_scene_graph:
            ...
            assert top_k_boxes.is_contiguous(), (
                "top_k_boxes not contiguous before scene_graph_encoder"
            )
            assert object_embeddings.is_contiguous(), (
                "object_embeddings not contiguous before scene_graph_encoder"
            )
            ...
            scene_graph_output = {
                ...
                "object_embeddings": object_embeddings,
                "batch": torch.zeros(
                    batch_size * top_k_scene, dtype=torch.long, device=det_feats.device
                ),
            }
        ...
        outputs["scene_graph.batch"] = (
            batch_sg
            if batch_sg is not None
            else torch.zeros(batch_size * top_k_scene, dtype=torch.long, device=det_feats.device)
        )
        ...
        scene_graph_invalid = (
            ...
            or edge_index.shape[1] != edge_attr.shape[0]
            or not edge_index.is_contiguous()
            or (
                edge_index.numel() > 0
                and edge_index.max().item() >= scene_graph_object_embeddings.shape[0]
            )
        )
        ...
        if scene_graph_invalid:
            ...
            if len(relations) > 0:
                spatial_relations = [
                    r
                    for r in relations
                    if r.predicate in self.scene_graph_encoder.spatial_predicates
                ]
                semantic_relations = [
                    r
                    for r in relations
                    if r.predicate not in self.scene_graph_encoder.spatial_predicates
                ]
        ...
        if self.training:
            ...
            if batch is not None:
                ...
                for b in range(num_scenes):
                    ...
                    scene_graphs.append(
                        {"relations": scene_relations, "object_embeddings": scene_object_embeddings}
                    )
        ...
        do_retrieval = (
            self.enable_retrieval and self.retrieval_system is not None and not skip_stage_b
        )
        ...
        if do_retrieval or do_scene_description:
            ...
            if do_retrieval:
                query_embeddings = {
                    "global": global_emb.detach().cpu().numpy()
                    if isinstance(global_emb, torch.Tensor)
                    else global_emb,
                }
            ...
            if do_scene_description:
                batch_size_for_regions = (
                    B_orig if temporal_mode and B_orig is not None else batch_size
                )
                ...
                batch_indices_regions = (
                    torch.arange(batch_size_for_regions, device=det_feats.device)
                    .unsqueeze(1)
                    .expand(-1, num_regions)
                )
                region_embs_tensor = det_feats[
                    batch_indices_regions, :, y_indices_regions, x_indices_regions
                ]

                region_embs_tensor = (
                    F.adaptive_avg_pool2d(
                        region_embs_tensor.unsqueeze(-1)
                        .unsqueeze(-1)
                        .contiguous()
                        .reshape(
                    ...
                    .squeeze(-1)
                    .contiguous()
                    .reshape(batch_size_for_regions, num_regions, region_embs_tensor.shape[2])
                )
        ...
        if user_id is not None:
            ...
            personalization = self.personalization_head(
                scene_features=scene_emb, user_id=user_id, interaction_features=None
            )
        ...
        if self.condition_mode == "color_blindness" and hasattr(self, "color_head"):
            ...
            color_logits = (
                color_logits.permute(0, 2, 3, 1).contiguous().reshape(batch_size, H * W, 12)
            )
    ...
    def get_detections(
        ...
        for b in range(batch_size):
            ...
            try:
                ...
                warnings.warn(
                    f"torchvision NMS not available, using original fallback: {e}", RuntimeWarning
                )
                nms_indices = torch.tensor(
                    self._nms(filtered_boxes, final_scores, nms_threshold),
            ...
            for idx in nms_indices:
                ...
                distance = (
                    DISTANCE_ZONES[dist_id] if 0 <= dist_id < len(DISTANCE_ZONES) else "medium"
                )
    ...
    def _get_urgency(
        ...
        if (
            ...
            and any(kw in class_lower for kw in self._high_priority_classes)
            and confidence > 0.3
        ):
    ...
    def apply_tier_config(self, tier_config: "TierConfig"):
        ...
        self._use_cross_task = (
            tier_config.use_cross_task_attention and self.cross_task_attention is not None
        )
        self._use_cross_modal = (
            tier_config.use_cross_modal_attention and self.cross_modal_attention is not None
        )
        self._use_attention = (
            tier_config.use_se_attention or tier_config.use_cbam_attention
        ) and self.fpn_attention is not None
...
class TierConfig:
    ...
    def from_dict(cls, d: dict) -> "TierConfig":
        ...
        try:
            tier_enum = (
                CapabilityTier[tier_name]
                if isinstance(tier_name, str)
                else CapabilityTier.T5_TEMPORAL
            )
        except KeyError:
        ...
        return cls(
            ...
            use_cross_task_attention=d.get(
                "use_cross_task_attention", base.use_cross_task_attention
            ),
            use_cross_modal_attention=d.get(
                "use_cross_modal_attention", base.use_cross_modal_attention
            ),
...
class TierManager:
    ...
    def __init__(self, mode: str = "patient", initial_tier: CapabilityTier | None = None):
