...
class ClassWeightedLoss(nn.Module):
    ...
    def _compute_weights(self, class_counts: dict[int, int], strategy: str) -> torch.Tensor:
    ...
    def update_weights(self, class_counts: dict[int, int], strategy: str = "inverse_freq"):
...
def load_pretrained_backbone(
    ...
    elif "efficientnet" in backbone_name or "mobilenet" in backbone_name:
...
def freeze_backbone(model: nn.Module, freeze: bool = True, unfreeze_last_n: int = 0):
...
def gradual_unfreeze_step(model: nn.Module, epoch: int, schedule: dict[int, int]):
...
def add_weight_decay(
    ...
    skip_list: tuple[str, ...] = ("bias", "bn", "norm"),
) -> list[dict]:
    ...
    def step(self, epoch: int):
        ...
        if epoch < self.warmup_epochs:
            ...
            if self.schedule == "linear":
                ...
                wd = (
                    self.final_wd
                    + (self.initial_wd - self.final_wd) * (1 + np.cos(np.pi * progress)) / 2
                )
            else:
                wd = self.initial_wd
...
class RegularizationManager:
    ...
    def __init__(
        ...
        if use_focal_loss:
            ...
            self.criterion = ClassWeightedLoss(class_counts=class_weights, num_classes=num_classes)
        else:
    ...
    def on_epoch_start(self, epoch: int, schedule: dict[int, int] | None = None):
...
def compute_class_weights_from_dataset(
    labels: list[int], num_classes: int, strategy: str = "inverse_freq"
) -> torch.Tensor:
    ...
    loss_fn = ClassWeightedLoss(
        class_counts=class_counts, num_classes=num_classes, weighting_strategy=strategy
    )
