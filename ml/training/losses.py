...
class ClassificationLoss(nn.Module):
    ...
    def forward(self, predictions: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        ce_loss = F.cross_entropy(
            predictions.reshape(-1, self.num_classes), targets.reshape(-1), reduction="none"
        )
...
class BoxRegressionLoss(nn.Module):
    ...
    def forward(self, predictions: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        smooth_l1 = torch.where(
            abs_diff < self.beta, 0.5 * diff**2 / self.beta, abs_diff - 0.5 * self.beta
        )
...
class DistanceZoneLoss(nn.Module):
    ...
    def __init__(self, num_zones: int = 3, class_weights: torch.Tensor | None = None):
        super().__init__()
...
class UncertaintyLoss(nn.Module):
    ...
    def forward(
        self, predictions: torch.Tensor, targets: torch.Tensor, prediction_variance: torch.Tensor
    ) -> torch.Tensor:
...
class DepthLoss(nn.Module):
    ...
    def forward(
        self, predictions: torch.Tensor, targets: torch.Tensor, uncertainty: torch.Tensor
    ) -> torch.Tensor:
...
class OCRLoss(nn.Module):
    ...
    def forward(
        ...
        return {"box_loss": box_loss, "text_loss": text_loss, "total_loss": box_loss + text_loss}
...
class MultiHeadLoss(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "loss_weights", torch.tensor([loss_weights.get(name, 1.0) for name in loss_functions])
        )
    ...
    def get_loss_weights(self) -> dict[str, float]:
        ...
        return {name: float(weights[i].item()) for i, name in enumerate(self._head_names)}
