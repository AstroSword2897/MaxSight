...
class FatigueLoss(HeadLoss):
    ...
    def __init__(
        self, fatigue_weight: float = 1.0, blink_weight: float = 0.5, fixation_weight: float = 0.5
    ):
        super().__init__()
...
class MotionLoss(HeadLoss):
    ...
    def forward(
        ...
        if self.use_edge_aware and image is not None:
            ...
            img_grad_x = torch.abs(image[:, :, :, :-1] - image[:, :, :, 1:]).mean(
                dim=1, keepdim=True
            )
            img_grad_y = torch.abs(image[:, :, :-1, :] - image[:, :, 1:, :]).mean(
                dim=1, keepdim=True
            )
...
class ROIPriorityLoss(HeadLoss):
    ...
    def forward(
        ...
        if target_rankings is not None and len(pred_scores) > 1:
            ...
            valid_mask = (rank_diff != 0) & ~torch.eye(
                N, dtype=torch.bool, device=pred_scores.device
            )
            if valid_mask.any():
...
def create_head_loss(head_type: str, **kwargs) -> HeadLoss:
    ...
    if head_type not in HEAD_LOSS_REGISTRY:
        ...
        raise ValueError(f"Unknown head loss type: '{head_type}'. Available types: {available}")
