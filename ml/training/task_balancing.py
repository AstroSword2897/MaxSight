...
class GradNormBalancer(nn.Module):
    ...
    def __init__(
        ...
        initial_task_weights: list[float] | None = None,
    ):
    ...
    def compute_gradient_norms(
        self, model: nn.Module, shared_params: list[nn.Parameter], task_losses: list[torch.Tensor]
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Compute gradient norms for each task."""
        # Apply learnable task weights to balance loss magnitudes.
        weighted_losses = [self.task_weights[i] * loss for i, loss in enumerate(task_losses)]
    ...
    def update_task_weights(
        self, task_losses: list[torch.Tensor], gradient_norms: torch.Tensor, iteration: int
    ) -> dict[str, float]:
        ...
        metrics = {
            ...
            "task_weights": {
                f"task_{i}": self.task_weights[i].item() for i in range(self.num_tasks)
            },
            "relative_losses": {
                f"task_{i}": relative_losses[i].item() for i in range(self.num_tasks)
            },
            "gradient_norms": {
                f"task_{i}": gradient_norms[i].item() for i in range(self.num_tasks)
            },
        }
...
class PCGradBalancer:
    ...
    def project_conflicting_gradients(self, gradients: list[torch.Tensor]) -> list[torch.Tensor]:
            ...
            for j, grad_j in enumerate(gradients):
                if i != j:
                    ...
                    if dot_product < 0:  # Gradients point in opposite directions.
                        ...
                        grad_i_proj = (
                            grad_i_proj - (dot_product / (grad_j.norm() ** 2 + 1e-8)) * grad_j
                        )
...
class GradNormMultiHeadLoss(nn.Module):
    ...
    def __init__(
        ...
        shared_params: list[nn.Parameter] | None = None,
        alpha: float = 1.5,
        ...
        initial_weights_tensor = torch.tensor(
            [initial_weights.get(name, 1.0) for name in self.head_names], dtype=torch.float32
        )
    ...
    def compute_head_losses(
        ...
        head_loss_dicts = {}
        device = next(
            (t.device for t in outputs.values() if torch.is_tensor(t)), torch.device("cpu")
        )
        aligned_pred, aligned_target = None, None
        if (
            "boxes" in outputs
            and "classifications" in outputs
            and "boxes" in targets
            and "labels" in targets
            and outputs["boxes"].dim() == 3
            and targets["boxes"].dim() == 3
        ):
            try:
        ...
        for head_name, loss_fn in self.head_losses.items():
            try:
                ...
                if pred is None or targ is None:
                    ...
                    pred = (
                        outputs.get(out_key)
                        if isinstance(outputs.get(out_key), torch.Tensor)
                        else None
                    )
                    targ = (
                        targets.get(targ_key)
                        if isinstance(targets.get(targ_key), torch.Tensor)
                        else None
                    )
                if pred is None or targ is None:
                    # Create zero loss with requires_grad=True for GradNorm compatibility.
    ...
    def compute_gradient_norms(
        self, model: nn.Module, head_losses: dict[str, torch.Tensor]
    ) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if len(self.shared_params) == 0:
            ...
            return torch.stack(head_loss_list), torch.ones(
                self.num_heads, device=head_loss_list[0].device
            )
        ...
        for i, weighted_loss in enumerate(weighted_losses):
            ...
            if not weighted_loss.requires_grad:
                logger.warning(
                    f"Head {self.head_names[i]} loss does not require grad, skipping gradient norm computation"
                )
        ...
        gradient_norms_tensor = torch.tensor(
            gradient_norms, device=weighted_losses_tensor.device, dtype=weighted_losses_tensor.dtype
        )
        return weighted_losses_tensor, gradient_norms_tensor

    def update_task_weights(
        self, head_losses: dict[str, torch.Tensor], gradient_norms: torch.Tensor
    ) -> dict[str, float]:
        ...
        metrics = {
            ...
            "avg_grad_norm": avg_grad_norm.item(),
            "task_weights": {
                name: self.task_weights[i].item() for i, name in enumerate(self.head_names)
            },
            "relative_losses": {
                name: relative_losses[i].item() for i, name in enumerate(self.head_names)
            },
            "gradient_norms": {
                name: gradient_norms[i].item() for i, name in enumerate(self.head_names)
            },
        }
        return metrics
    ...
    def forward(
        ...
        metrics = {
            ...
            "head_losses": {
                name: loss.item() if torch.is_tensor(loss) else loss
