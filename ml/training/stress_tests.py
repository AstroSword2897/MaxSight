...
class StressTestConfig:
    ...
    # Head isolation.
    head_isolation_variants: list[list[str]] = field(
...
class HeadIsolationStressTest:
    ...
    def _train_variant(
        self, model: nn.Module, train_loader: Any, val_loader: Any, device: str, epochs: int
    ) -> dict[str, list[float]]:
    ...
    def _evaluate_variant(
        self, variant_name: str, metrics: dict[str, list[float]]
    ) -> StressTestResult:
...
class LossScalingStressTest:
    ...
    def _create_scaled_loss(
        self, loss_fn: nn.Module, head_name: str, scale_factor: float
    ) -> nn.Module:
...
class InputCorruptionStressTest:
    ...
    def run(
        self, model: nn.Module, val_loader: Any, device: str = "cuda"
    ) -> dict[str, StressTestResult]:
        ...
        for corruption_type in self.config.corruption_types:
            ...
            result = self._test_corruption(model, val_loader, device, corruption_type)
    ...
    def _test_corruption(
        self, model: nn.Module, val_loader: Any, device: str, corruption_type: str
    ) -> StressTestResult:
    ...
    def _apply_corruption(self, images: torch.Tensor, corruption_type: str) -> torch.Tensor:
...
class TemporalStressTest:
    ...
    def run(self, model: nn.Module, device: str = "cuda") -> StressTestResult:
        ...
        with torch.no_grad():
            ...
            for frame_idx in range(self.config.temporal_test_frames):
                ...
                if "urgency" in outputs:
                    urgency = (
                        outputs["urgency"].item()
                        if torch.is_tensor(outputs["urgency"])
                        else outputs["urgency"]
                    )
                ...
                if prev_outputs is not None:
                    ...
                    if "urgency" in outputs and "urgency" in prev_outputs:
                        urgency_diff = abs(
                            urgency_values[-1] - urgency_values[-2]
                            if len(urgency_values) > 1
                            else 0
                        )
        ...
        if len(distance_values) > 10:
            distance_variance = np.var(
                [d.item() if torch.is_tensor(d) else d for d in distance_values]
            )
...
class HeadDropoutStressTest:
    ...
    def run(
        ...
        for head_name in self.config.dropout_heads:
            ...
            result = self._test_dropout(model, val_loader, device, head_name)
    ...
    def _test_dropout(
        self, model: nn.Module, val_loader: Any, device: str, head_name: str
    ) -> StressTestResult:
...
class StressTestSuite:
    ...
    def run_all(
        ...
        return {"results": all_results, "dashboard": dashboard}
