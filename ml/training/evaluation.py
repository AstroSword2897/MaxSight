...
def generate_evaluation_report(metrics: dict[str, float], save_path: Path | None = None) -> str:
    ...
    available_lightings = [
        l
        for l in default_lighting_conditions
        if any(f"{l}_{m}" in metrics for m in ["precision", "recall", "f1"])
    ]
    ...
    if normal_recall > 0:
        for lighting in ["bright", "dim", "dark"]:
            ...
            recall_degradation = (
                ((normal_recall - lighting_recall) / normal_recall) * 100
                if normal_recall > 0
                else 0.0
            )
            precision_degradation = (
                ((normal_precision - lighting_precision) / normal_precision) * 100
                if normal_precision > 0
                else 0.0
            )
...
def plot_lighting_metrics(metrics: dict[str, float], save_path: Path | None = None) -> None:
    ...
    ax1.bar(
        [i - width / 2 for i in x], precisions, width, label="Precision", color="#3498db", alpha=0.8
    )
...
def analyze_lighting_degradation(
    metrics: dict[str, float], lighting_conditions: list[str] | None = None
