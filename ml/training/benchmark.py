...
def benchmark_inference(
    ...
    input_shapes: dict[int, tuple] | None = None,
    ...
    for batch_size in batch_sizes:
        ...
        batch_results["fps"] = (
            1000.0 / batch_results["mean_ms"] if batch_results["mean_ms"] > 0 else 0.0
        )
...
def log_benchmark_results(results: dict[str, dict[str, float]]) -> None:
    ...
    for key, stats in results.items():
        if key == "overall":
            continue
        logger.info(
            "Batch %s: mean=%.2f ms, median=%.2f ms, min=%.2f, max=%.2f, std=%.2f",
            key.replace("batch_", ""),
            stats["mean_ms"],
            stats["median_ms"],
            stats["min_ms"],
    ...
    if "overall" in results:
        ...
        logger.info(
            "Overall (batch_size=1): mean=%.2f ms, target <500 ms: %s", o["mean_ms"], status
        )
