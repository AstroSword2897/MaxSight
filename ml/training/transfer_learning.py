...
class TierTransferManager:
    ...
    def __init__(
        self, source_checkpoint: Path, target_model: nn.Module, transfer_config: dict[str, Any]
    ):
    ...
    def transfer_weights(self, strict: bool = False) -> dict[str, int]:
        ...
        logger.info(
            f"Transfer complete: {transferred} transferred, {skipped} skipped, "
            f"{shape_mismatch} shape mismatches"
        )

        return stats

    def create_parameter_groups(self, base_lr: float) -> list[dict[str, Any]]:
        ...
        if detection_params:
            param_groups.append(
                {"params": detection_params, "lr": detection_lr, "name": "detection_heads"}
            )
        if temporal_params:
    ...
    def get_freeze_schedule(self, epoch: int) -> dict[str, bool]:
        ...
        for name, param in self.target_model.named_parameters():
            ...
            if epoch < 5:
                ...
                should_freeze = not is_new_head and any(
                    x in name
                    for x in [
                        "backbone",
                        "vit",
                ...
                if is_new_head or any(x in name for x in ["detection_head", "classification_head"]):
                    should_freeze = False
                else:
                    should_freeze = any(x in name for x in ["backbone", "vit", "transformer"])
            elif epoch < 30:
    ...
    def _extract_layer_number(self, name: str) -> int | None:
...
def create_transfer_optimizer(
    model: nn.Module, base_lr: float, weight_decay: float = 0.05
) -> torch.optim.AdamW:
