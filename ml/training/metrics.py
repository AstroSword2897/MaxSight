...
class DetectionMetrics:
        ...
        for idx in sorted_indices:
            ...
            if not class_match.any():
                ...
                if self.store_predictions:
                    self.class_predictions[pred_class].append(
                        DetectionPrediction(
                            pred_score, 0.0, None, float(pred_areas[pred_idx].item())
                        )
                    )
                continue
            ...
            if is_tp:
                ...
                if self.store_predictions:
                    self.class_predictions[pred_class].append(
                        DetectionPrediction(
                            pred_score,
                            best_iou,
                            int(best_gt_idx.item()),
                            float(pred_areas[pred_idx].item()),
                        )
                    )
            else:
                self.class_fp[pred_class] += 1
                ...
                if self.store_predictions:
                    self.class_predictions[pred_class].append(
                        DetectionPrediction(
                            pred_score, best_iou, None, float(pred_areas[pred_idx].item())
                        )
                    )
    ...
    def compute_precision(self, class_idx: int | None = None) -> float:
    ...
    def compute_recall(self, class_idx: int | None = None) -> float:
    ...
    def compute_f1(self, class_idx: int | None = None) -> float:
    ...
    def compute_map_vectorized(self, iou_threshold: float | None = None) -> dict[str, float]:
        ...
        per_threshold = {
            float(thresh): float(
                np.mean(
                    [self.compute_ap_vectorized(c, float(thresh)) for c in range(self.num_classes)]
                )
            )
        ...
        result = {
            ...
            "per_class": [
                self.compute_ap_vectorized(c, float(thresholds[0])) for c in range(self.num_classes)
            ],
        }
    ...
    def get_latency_stats(self) -> dict[str, float]:
        ...
        if not self.inference_times:
            return {
                k: 0.0
                for k in ["mean_ms", "median_ms", "min_ms", "max_ms", "std_ms", "p95_ms", "p99_ms"]
            }
    ...
    def compute_map(self, iou_threshold: float | None = None) -> dict[str, float]:
