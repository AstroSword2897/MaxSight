...
def validate_model_inputs(images: torch.Tensor, expected_shape: tuple[int, ...] = (4,)) -> bool:
    ...
    if images.dim() != len(expected_shape):
        raise ValueError(f"Images must have {len(expected_shape)} dimensions, got {images.dim()}")
...
def validate_model_outputs(
    outputs: dict[str, torch.Tensor], expected_keys: list | None = None
) -> bool:
...
def validate_batch(
    batch: Any, required_keys: list | None = None
...
def validate_checkpoint(checkpoint: dict[str, Any], required_keys: list | None = None) -> bool:
