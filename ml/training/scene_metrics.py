...
class SceneMetrics:
    ...
    def reset(self):
        ...
        self.urgency_confusion = torch.zeros(
            self.num_urgency_levels, self.num_urgency_levels, dtype=torch.long
        )
        ...
        self.distance_confusion = torch.zeros(
            self.num_distance_zones, self.num_distance_zones, dtype=torch.long
        )
    ...
    def update_urgency(self, pred_urgency: torch.Tensor, gt_urgency: torch.Tensor) -> None:
        ...
        valid_mask = (
            (pred_flat >= 0)
            & (pred_flat < self.num_urgency_levels)
            & (gt_flat >= 0)
            & (gt_flat < self.num_urgency_levels)
        )
        ...
        if len(pred_valid) > 0:
            ...
            self.urgency_confusion.reshape(-1).index_add_(
            ...
            for g_idx in gt_valid.unique():
                ...
                self.urgency_level_correct[g_idx_int] += int(
                    ((pred_valid == g_idx_int) & (gt_valid == g_idx_int)).sum().item()
                )
    ...
    def update_distance(
        ...
        valid_mask = (
            (pred_flat >= 0)
            & (pred_flat < self.num_distance_zones)
            & (gt_flat >= 0)
            & (gt_flat < self.num_distance_zones)
        )
        pred_valid = pred_flat[valid_mask]
        gt_valid = gt_flat[valid_mask]
        ...
        if len(pred_valid) > 0:
            ...
            self.distance_confusion.reshape(-1).index_add_(
                0, indices, torch.ones_like(indices, dtype=torch.long)
            )
    ...
    def get_normalized_confusion_matrices(self) -> dict[str, torch.Tensor]:
        ...
        return {"urgency": urgency_norm, "distance": distance_norm}
