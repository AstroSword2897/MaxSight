...
class EvaluationMetrics:
    ...
    def compute_accessibility_metrics(
        ...
        if ground_truth_detections:
            detected = sum(
                ...
                if any(self._iou_overlap(gt["box"], det["box"]) > 0.5 for det in detections)
            )
        ...
        if detections:
            false_positives = sum(
                ...
                if not any(
                    self._iou_overlap(det["box"], gt["box"]) > 0.5 for gt in ground_truth_detections
                )
            )
    ...
    def compute_robustness_metrics(
        ...
        if lighting_performance:
            ...
            metrics.lighting_robustness = (
                avg_lighting / baseline_performance if baseline_performance > 0 else 0.0
            )

        # Occlusion robustness.
        if occlusion_performance:
            avg_occlusion = np.mean(list(occlusion_performance.values()))
            metrics.occlusion_robustness = (
                avg_occlusion / baseline_performance if baseline_performance > 0 else 0.0
            )
        ...
        if noise_performance:
            ...
            metrics.noise_robustness = (
                avg_noise / baseline_performance if baseline_performance > 0 else 0.0
            )
