...
class PatchExtractor(nn.Module):
    ...
    def __init__(
        self,
        vit_backbone: nn.Module | None = None,
        num_clusters: int = 25,
        embed_dim: int = 768,
        num_heads: int = 8,
        ...
        self.cluster_tokens = nn.Parameter(torch.randn(1, num_clusters, embed_dim) * 0.02)
        ...
        if use_soft_kmeans:
            ...
            self.soft_cluster_centers = nn.Parameter(torch.randn(1, num_clusters, embed_dim) * 0.02)

    def _attention_pooling(self, patch_tokens: torch.Tensor) -> torch.Tensor:
    ...
    def _soft_kmeans(self, patch_tokens: torch.Tensor) -> torch.Tensor:
        ...
        cluster_centers = self.soft_cluster_centers.expand(
            B, -1, -1
        )  # [B, num_clusters, embed_dim].
    ...
    def forward(
        self, images: torch.Tensor, vit_patch_tokens: torch.Tensor | None = None
    ) -> torch.Tensor:
        ...
        if self.use_soft_kmeans:
            ...
            if self.use_gradient_checkpointing and self.training:
                ...
                clustered_embeddings = torch.utils.checkpoint.checkpoint(
                    self._attention_pooling, vit_patch_tokens, use_reentrant=False
                )
            else:
    ...
    def get_attention_weights(
        ...
        _, attention_weights = self.attention_pool(
            query=cluster_tokens, key=patch_tokens_proj, value=patch_tokens_proj
        )
