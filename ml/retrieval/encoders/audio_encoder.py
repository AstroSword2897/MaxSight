...
class AudioEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.spatial_head = nn.Sequential(nn.Linear(embed_dim, 128), nn.ReLU(), nn.Linear(128, 6))

    def forward(
        self,
        audio_features: torch.Tensor,  # [B, T, input_dim] or [B, input_dim].
        stereo_channels: torch.Tensor | None = None,  # [B, T, 2].
        ...
        if audio_features.dim() == 3:
            ...
            # Simple projection.
            audio_embed = nn.Linear(audio_features.shape[-1], self.embed_dim).to(
