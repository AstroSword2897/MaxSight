...
class RegionExtractor(nn.Module):
    ...
    def extract_regions(
        ...
        boxes: torch.Tensor | None = None,  # [B, N, 4] (x1, y1, x2, y2)
        ...
        for b in range(B):
            ...
            for i in range(min(self.max_regions, boxes.shape[1])):
                ...
                region = F.interpolate(
                    region, size=self.region_size, mode="bilinear", align_corners=False
                )
    ...
    def _create_grid_regions(self, B: int, H: int, W: int, device: torch.device) -> torch.Tensor:
