    ...
    def _cache_result(self, request_id: str, result: RetrievalResult):
        ...
        with self.lock:
            ...
            if len(self.result_cache) >= self.cache_max_size:
                # Remove oldest entry.
                oldest_key = min(
                    self.result_cache.keys(), key=lambda k: self.result_cache[k]["timestamp"]
                )
                del self.result_cache[oldest_key]

            self.result_cache[request_id] = {"result": result, "timestamp": time.time()}

    def get_cached_result(self, request_id: str) -> RetrievalResult | None:
