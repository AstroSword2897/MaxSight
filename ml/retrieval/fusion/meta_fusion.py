...
class MetaFusionWeights(nn.Module):
    ...
    def __init__(
        ...
        self.meta_learner = nn.Sequential(
            nn.Linear(
                num_modalities + 4, hidden_dim
            ),  # Modalities + task_type + urgency + confidence + user_embed.
            nn.ReLU(),
    ...
    def forward(
        ...
        confidence_tensor = torch.full(
            (B, 1), confidence if confidence is not None else 0.5, device=device
        )
...
class ActiveSceneExploration(nn.Module):
    ...
    def forward(
        ...
        urgency: torch.Tensor | None = None,  # [B].
        ...
        pref_tensor = torch.full(
            (B, 1), user_preference if user_preference is not None else 0.5, device=device
        )
...
class PredictiveNavigationGuidance(nn.Module):
    ...
    def forward(
        ...
        scene_context: torch.Tensor | None = None,  # [B, embed_dim].
