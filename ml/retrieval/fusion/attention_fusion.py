"""Attention-Based Fusion for Multi-Vector Retrieval Query-adaptive attention fusion of multiple embedding types."""
...
class AttentionFusion(nn.Module):
    """Query-adaptive attention fusion. Learns attention weights per embedding type dynamically based on query."""
    ...
    def __init__(
        ...
        self.projections = nn.ModuleDict(
            {name: nn.Linear(dim, fused_dim) for name, dim in embedding_dims.items()}
        )

        # Attention mechanism.
        self.attention = nn.MultiheadAttention(fused_dim, num_heads, batch_first=True)

        # Query generator (learns what to attend to)
        self.query_generator = nn.Sequential(
            nn.Linear(fused_dim, fused_dim), nn.ReLU(), nn.Linear(fused_dim, fused_dim)
        )
    ...
    def forward(
        ...
        query_embedding: torch.Tensor | None = None,  # [B, fused_dim].
    ) -> torch.Tensor:
        ...
        fused, attn_weights = self.attention(
            query=query, key=all_embeddings, value=all_embeddings
        )  # [B, 1, fused_dim].
