...
def test_class_system():
    ...
    assert len(COCO_BASE_CLASSES) > 0, (
        f"COCO base classes should exist, got {len(COCO_BASE_CLASSES)}"
    )
...
def test_model_creation():
    ...
    assert model.num_classes == len(COCO_CLASSES), (
        f"Model classes {model.num_classes} != {len(COCO_CLASSES)}"
    )
    ...
    assert cls_head_last.out_channels == len(COCO_CLASSES), (
        f"Classification head mismatch: {cls_head_last.out_channels} != {len(COCO_CLASSES)}"
    )
...
def test_data_sources():
    ...
    assert len(ACCESSIBILITY_CLASSES) > 0, (
        f"Accessibility classes should exist, got {len(ACCESSIBILITY_CLASSES)}"
    )
