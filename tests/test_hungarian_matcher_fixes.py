...
def test_matching_empty_gt():
    ...
    indices, costs = match_predictions_to_gt(pred_boxes, pred_logits, gt_boxes, gt_labels)
...
def test_matching_valid_inputs():
    ...
    pred_boxes = torch.tensor([[0.3, 0.3, 0.2, 0.2], [0.7, 0.7, 0.15, 0.15]])
    pred_logits = torch.randn(2, 91)
    gt_boxes = torch.tensor([[0.3, 0.3, 0.2, 0.2], [0.7, 0.7, 0.15, 0.15]])
...
def test_matching_batch_mixed():
    ...
    indices_list, costs_list = match_batch(pred_boxes, pred_logits, gt_boxes, gt_labels)
...
def test_validate_and_sanitize_batch():
    ...
    clean_batch, is_valid, msg = validate_and_sanitize_batch(batch, num_classes=91, auto_fix=True)
