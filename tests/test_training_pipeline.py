...
from ml.training.losses import BoxRegressionLoss, ClassificationLoss, ObjectnessLoss
...
class DetectionLoss(nn.Module):
    ...
    def forward(self, predictions: dict, targets: dict) -> dict:
        ...
        if "labels" in targets:
            labels_list = (
                targets["labels"] if isinstance(targets["labels"], list) else [targets["labels"]]
            )
            labels_batched = torch.full(
                (B, N), -1, dtype=torch.long, device=predictions["classifications"].device
            )
        ...
        if "boxes" in targets:
            boxes_list = (
                targets["boxes"] if isinstance(targets["boxes"], list) else [targets["boxes"]]
            )
        ...
        if "objectness" in predictions and objectness_targets is not None:
            losses["objectness"] = self.objectness_loss(
                predictions["objectness"], objectness_targets
            )
        ...
        if "classifications" in predictions and labels_batched is not None:
            ...
            if valid_mask.any():
                valid_preds = predictions["classifications"][
                    valid_mask
                ...
                losses["classification"] = torch.tensor(
                    0.0, device=predictions["classifications"].device
                )
        ...
        if "boxes" in predictions and boxes_batched is not None:
            ...
            valid_mask = (
                (labels_batched >= 0)
                if labels_batched is not None
                else torch.ones(B, N, dtype=torch.bool, device=predictions["boxes"].device)
            )
            if valid_mask.any():
