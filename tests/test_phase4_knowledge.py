...
class TestSceneGraphEncoder:
    ...
    def test_scene_graph_forward(self):
        ...
        encoder = SceneGraphEncoder(object_embed_dim=256, relation_embed_dim=128)
        encoder.eval()
...
class TestGNNEncoder:
    ...
    def test_gnn_forward(self):
        ...
        try:
            encoder = GNNEncoder(
                node_dim=256, edge_dim=128, hidden_dim=256, num_layers=3, output_dim=512
            )
            encoder.eval()

            # Create dummy graph.
            node_features = torch.randn(5, 256)  # [N, D].
            edge_index = torch.tensor([[0, 1, 2, 3], [1, 2, 3, 4]], dtype=torch.long)  # [2, E].
...
class TestKnowledgeAugment:
    ...
    def test_knowledge_augment_forward(self):
        ...
        try:
            ...
            retrieval = KnowledgeAugmentedRetrieval(node_dim=256, embed_dim=512)
            retrieval.eval()
