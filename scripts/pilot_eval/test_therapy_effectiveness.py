...
from ml.therapy.therapy_integration import create_therapy_integrator
...
def run_therapy_session_simulation(
    session_duration: int = 20, initial_skill: float = 0.5, skill_improvement_rate: float = 0.02
) -> dict:
    ...
    session_id = session_mgr.start_session(
        {"initial_skill": initial_skill, "duration": session_duration}
    )
    ...
    for task_num in range(session_duration):
        ...
        recent_performance = (
            session_mgr.task_attempts[-5:]
            if len(session_mgr.task_attempts) >= 5
            else session_mgr.task_attempts
        )

        task = task_gen.generate_task(
            uncertainty=uncertainty, fatigue_score=fatigue, recent_performance=recent_performance
        )

        # Simulate user performing task.
        result = simulate_user_performance(
            difficulty=task["difficulty"], user_skill=user_skill, fatigue=fatigue
        )

        # Log attempt.
        session_mgr.log_task_attempt(
            task_type=task["task_type"].value, task_config=task, result=result
        )
        ...
        if task["task_type"] == TaskType.FATIGUE_REST:
            ...
            print(
                f"  Task {task_num + 1:2d}: {task['task_type'].value:20s} | Difficulty: {task['difficulty']:.2f} | {status} | Skill: {user_skill:.2f} | Fatigue: {fatigue:.2f}"
            )
...
def test_adaptive_difficulty():
    ...
    task1 = task_gen.generate_task(
        uncertainty=0.8, fatigue_score=0.3, recent_performance=poor_performance
    )
    ...
    task2 = task_gen.generate_task(
        uncertainty=0.2, fatigue_score=0.1, recent_performance=good_performance
    )
    ...
    print("\nScenario 3: User is fatigued")
    task3 = task_gen.generate_task(
        uncertainty=0.5, fatigue_score=0.85, recent_performance=good_performance
    )
...
def test_skill_progression():
    ...
    for i, result in enumerate(results):
        ...
        print(
            f"{i + 1:<10} {summary['success_rate']:<15.2%} "
...
def test_therapy_with_real_scene():
    ...
    scene_description = (
        "Busy street intersection with pedestrian crossing, moving vehicles, and traffic signals"
    )
    ...
    task_types = [
        (
            "attention",
            lambda: integrator.create_attention_task(
        ...
        (
            "contrast",
            lambda: integrator.create_contrast_task(
                scene_description=scene_description, contrast_levels=[0.9, 0.5, 0.2], difficulty=0.6
            ),
        ),
        (
            "edge",
            lambda: integrator.create_edge_task(
                scene_description=scene_description,
        ...
        (
            "spatial",
            lambda: integrator.create_spatial_task(
                scene_description=scene_description,
...
def test_long_term_effectiveness():
    ...
    for s in session_summaries:
        print(
            f"{s['session']:<10} {s['success_rate']:<15.2%} "
    ...
    print("\nIMPROVEMENT METRICS:")
    print(
        f"  Success rate: {first_session['success_rate']:.2%} -> {last_session['success_rate']:.2%} (Δ {success_improvement:+.2%})"
    )
    print(
        f"  Skill level: {first_session['skill_level']:.2f} -> {last_session['skill_level']:.2f} (Δ {skill_improvement:+.2f})"
    )
    print(
        f"  Reaction time: {first_session['avg_reaction_time']:.2f}s -> {last_session['avg_reaction_time']:.2f}s (Δ {time_improvement:+.2f}s)"
    )
    ...
    if success_improvement > 0:
        ...
        print(
            f"  INFO  Success rate change: {success_improvement:.1%} (may vary due to adaptive difficulty)"
        )
    ...
    if time_improvement >= 0:
        ...
        print(
            f"  INFO  Reaction time change: {time_improvement:+.2f}s (expected with increased difficulty)"
        )
