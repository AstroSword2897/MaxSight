    ...
    def patch_split(self, split: str, max_workers: int = 4) -> tuple[int, int]:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for i, future in enumerate(as_completed(futures), 1):
                ...
                if i % 50 == 0:
                    logger.info(
                        f"Progress: {i}/{len(missing_files)} "
...
def main():
    parser = argparse.ArgumentParser(description="Download missing COCO images to complete dataset")
    parser.add_argument(
        "--split",
    ...
    parser.add_argument(
        "--workers", type=int, default=4, help="Number of parallel download threads (default: 4)"
    )
    parser.add_argument(
        "--root",
        type=Path,
