...
def download_file(url: str, dest: Path, resume: bool = True) -> bool:
    ...
    try:
        ...
        with open(dest, mode) as f:
            ...
            with tqdm(
                total=total_size, unit="B", unit_scale=True, desc=f"  Downloading {dest.name}"
            ) as pbar:
...
def download_open_images_v6(data_dir: Path) -> bool:
    ...
    try:
        ...
        if download_file(csv_url, csv_path):
            print("  [ok] Annotation CSV downloaded")
        ...
        if img_count > 0 and csv_path.exists():
            ...
            print("  WARNING Download incomplete - some files missing")
            return False
    ...
    if download_file(csv_url, csv_path):
        print("  [ok] Annotation CSV downloaded")
        print("  WARNING Images still need to be downloaded manually")
        return False
    else:
        print("  WARNING Annotation CSV download failed (manual download required)")
        return False
...
def download_bdd100k(data_dir: Path) -> bool:
    ...
    images_url = (
        "https://dl.cv.ethz.ch/bdd100k/data/100k_images_val.zip"  # 542MB - validation images only.
    )
    ...
    print("  Validation images: 542MB")
    print("  Detection labels: 53MB")
    ...
    if download_file(labels_url, labels_zip):
        print("  [ok] Labels downloaded, extracting...")
        ...
        if extract_zip(labels_zip, temp_labels):
            ...
            if det_val_source and det_val_source.exists():
                ...
                print("  WARNING Could not find det_val.json in extracted files")
        ...
        print("  WARNING Labels download failed")
        labels_success = False
    ...
    if download_file(images_url, images_zip):
        print("  [ok] Images downloaded, extracting...")
        # Extract to temp location.
        ...
        if extract_zip(images_zip, temp_extract):
            ...
            if val_source and val_source.exists():
                ...
                print("  WARNING Could not find val folder with images")
        ...
        print("  WARNING Images download failed")
        images_success = False
    ...
    if not images_success or not labels_success:
        print("\n" + "=" * 70)
        print("WARNING  BDD100K Download Failed")
        print("=" * 70)
        print("\nPossible reasons:")
        print("  - DNS resolution failed (dl.cv.ethz.ch not accessible)")
        print("  - Network/firewall blocking access")
        print("  - Server temporarily unavailable")

        print("\nAlternative Options:")
        print("\n1. Use VPN/Proxy:")
        print("   - Enable VPN that can access ETH Zurich domains")
        print("   - Retry this script or download manually")

        print("\n2. Manual Download (if VPN works):")
        print("   - Visit: https://dl.cv.ethz.ch/bdd100k/data/")
        if not images_success:
            print("   - Download: '100k_images_val.zip' (542MB)")
            print(f"   - Extract val folder to: {images_dir / 'val'}")
        if not labels_success:
            print("   - Download: 'bdd100k_det_20_labels_trainval.zip' (53MB)")
            print(f"   - Extract det_val.json to: {labels_path}")

        print("\n3. Alternative Sources:")
        print("   - Hugging Face: https://huggingface.co/datasets/kd7/graid-bdd100k")
        print("   - FiftyOne: https://docs.voxel51.com/dataset_zoo/datasets/bdd100k.html")
        print("   - GitHub: https://github.com/bdd100k/bdd100k")

        print("\n4. Skip for Now:")
        print("   - ADE20K is already downloaded OK (indoor scenes)")
        print("   - BDD100K is for outdoor/driving scenarios - can add later")
    ...
    if download_file(urls["validation_images"], zip_path):
        ...
        if extract_zip(zip_path, temp_extract):
            ...
            if ade_data.exists():
                ...
                print("  [ok] ADE20K extracted successfully")
                return True

    print("  [fail] ADE20K download/extraction failed")
    return False


def verify_dataset(dataset_name: str, data_dir: Path) -> dict[str, bool]:
    """Verify that a dataset is properly downloaded."""
    status = {"images": False, "annotations": False, "complete": False}
...
def main():
    parser = argparse.ArgumentParser(description="Download all inference datasets for MaxSight")
    parser.add_argument(
        "--base-dir",
        type=Path,
    ...
    parser.add_argument(
        "--skip-open-images", action="store_true", help="Skip Open Images V6 download"
    )
    parser.add_argument("--skip-bdd100k", action="store_true", help="Skip BDD100K download")
    parser.add_argument("--skip-ade20k", action="store_true", help="Skip ADE20K download")
    parser.add_argument(
        "--verify-only", action="store_true", help="Only verify existing datasets, do not download"
    )
