    ...
    try:
        while True:
            ...
            if fo_dir.exists():
                ...
                total_size = sum(f.stat().st_size for f in fo_dir.rglob("*.jpg") if f.is_file()) / (
                    1024**2
                )  # MB.

                print(
                    f"\r[{time.strftime('%H:%M:%S')}] Downloaded: {img_count:,} images ({total_size:.1f} MB)",
                    end="",
                    flush=True,
                )
                ...
                print(
                    f"\r[{time.strftime('%H:%M:%S')}] Waiting for download to start...",
                    end="",
                    flush=True,
                )
        ...
        print("  tail -f /tmp/open_images_download.log")
