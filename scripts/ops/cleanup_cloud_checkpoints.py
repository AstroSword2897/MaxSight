...
def cleanup_logs(log_dir: Path = Path("logs"), dry_run: bool = True) -> tuple[int, float]:
    ...
    response = (
        input(f"\nDelete {len(log_files)} log file(s) ({total_size:.2f} MB)? (yes/no): ")
        .strip()
        .lower()
    )
    if response != "yes":
...
def main():
    ...
    parser.add_argument(
        "--keep-best", action="store_true", default=True, help="Keep best_model.pt (default: True)"
    )
    parser.add_argument(
        "--keep-last",
        action="store_true",
    ...
    parser.add_argument(
        "--keep-recent", type=int, default=0, help="Keep N most recent checkpoints (default: 0)"
    )
    parser.add_argument("--clean-logs", action="store_true", help="Also clean log files")
    parser.add_argument(
        "--clean-temp",
        action="store_true",
