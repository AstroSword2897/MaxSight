...
def clean_docstring(docstring: str) -> str:
    ...
    for line in lines:
        ...
        if any(phrase in line_lower for phrase in ['why this', 'design philosophy', 'how it connects', 
                                                   'relationship to', 'technical design decision',
                                                   'project philosophy', 'barrier removal']):
            skip_section = True
            continue
...
def clean_file_comments(content: str) -> str:
    ...
    for line in lines:
        ...
        if stripped.startswith('#'):
            if any(phrase in stripped.lower() for phrase in ['todo', 'fixme', 'hack', 'xxx', 'note:', 'warning:', 'important:']):
                cleaned.append(line)
            elif len(stripped) > 80:
                continue
            elif any(phrase in stripped.lower() for phrase in ['location:', 'assumes', 'does not handle', 'not tested']):
                continue
            else:
                cleaned.append(line)
            ...
            if any(phrase in comment.lower() for phrase in ['type:', 'type ignore', 'noqa', 'fmt:']):
                cleaned.append(line)
            elif len(comment) > 60:
                cleaned.append(code_part)
            elif any(phrase in comment.lower() for phrase in ['serialize', 'thread-safe', 'kill switch', 'hard disable']):
...
def main():
    ...
    if target.is_file():
        ...
        files = [f for f in files if '.git' not in str(f) and 'venv' not in str(f) and '__pycache__' not in str(f)]
    ...
    for file_path in sorted(files):
        ...
        if changed:
            cleaned_count += 1
            print(f"  {file_path.relative_to(target.parent if target.is_file() else target)}: {status}")
