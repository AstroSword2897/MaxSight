...
def _script_path(name: str) -> Path:
    """Resolve script path: scripts/ first, then scripts/archive/."""
    p = ROOT / "scripts" / name
    if p.exists():
        return p
    return ROOT / "scripts" / "archive" / name
...
def main() -> int:
    ...
    if not args.skip_download:
        ...
        download_script = _script_path("download_coco.py")
        if not download_script.exists():
            print("  Warning: download_coco.py not found in scripts/ or scripts/archive/")
        else:
            try:
                run(
                    [
