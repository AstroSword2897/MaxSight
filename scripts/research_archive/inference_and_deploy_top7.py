...
if not _SCRIPT.exists() or not (REPO / ".git").exists():
    print(
        "Repo not found: run the clone and cd first (e.g. in Colab run Cell 1 before this script).",
        file=sys.stderr,
    )
    if Path("/content").exists():
        ...
        print(
            "  cd /path/to/2026-Prototype  then  python scripts/inference_and_deploy_top7.py ...",
            file=sys.stderr,
        )
    sys.exit(1)
...
def main():
    ...
    p.add_argument("--skip-inference", action="store_true", help="Skip inference; only deploy")
    p.add_argument("--skip-deploy", action="store_true", help="Skip deploy; only run inference")
    p.add_argument(
        "--top-by-map",
        action="store_true",
    ...
    p.add_argument(
        "--quick", action="store_true", help="Deploy with JIT-only export (faster, skip ExecuTorch)"
    )
    p.add_argument("--quiet", action="store_true", help="Less output")
    args = p.parse_args()

    def _is_placeholder(p: Path | None) -> bool:
        if p is None:
            return True
        s = str(p)
    ...
    if base is None:
        ...
        if base is None:
            ...
            if Path("/content/drive").exists():
                print(
                    "On Colab use: --checkpoints-base /content/drive/MyDrive/MaxSight",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    "Pass --checkpoints-base <dir> or set CHECKPOINTS_BASE. Discover: python scripts/find_trained_checkpoints.py",
                    file=sys.stderr,
                )
            return 1
    base = base.resolve()
    ...
    run_inference = (
        (not args.skip_inference) and args.val_annotation and args.val_annotation.exists()
    )
    if args.val_annotation and not args.val_annotation.exists():
        print(
            f"Warning: val-annotation not found {args.val_annotation}; skipping inference.",
            file=sys.stderr,
        )
        run_inference = False
    ...
    if top_by_map and not inference_data_path.exists() and run_inference:
        ...
        if r.returncode != 0:
            print(
                "Inference step had errors; continuing to deploy with default top 7.",
                file=sys.stderr,
            )
        else:
