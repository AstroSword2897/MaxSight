...
def verify_mlpackage(mlpackage_path: Path):
    ...
    if missing:
        ...
        print("   This might be an incomplete conversion")
        return False
    ...
    try:
        ...
        print(
            f"  Size: {sum(f.stat().st_size for f in mlpackage_path.rglob('*') if f.is_file()) / (1024 * 1024):.1f} MB"
        )
...
def main():
    if len(sys.argv) < 2:
        ...
        print("\nExample:")
        print(
            "  python scripts/verify_coreml.py checkpoints/checkpoints_color_blindness/best_model.mlpackage"
        )
        return 1
