...
def main():
    ...
    parser.add_argument(
        "--epochs", type=int, default=55, help="Total epochs (5 warmup + 50 training)"
    )
    ...
    parser.add_argument(
        "--num-workers", type=int, default=2, help="DataLoader workers (Colab often 0-2)"
    )
    ...
    train_loader = DataLoader(
        ...
        if hasattr(train_loader_full, "collate_fn")
        else None,
    )
    ...
    logger.info(
        f"Model: T5_TEMPORAL, {sum(p.numel() for p in model.parameters()) / 1e6:.2f}M parameters"
    )
    ...
    try:
        logger.info(
            f"Starting T5 training: {args.epochs} epochs, warmup {args.warmup_epochs}, subset {fraction * 100:.1f}%"
        )
