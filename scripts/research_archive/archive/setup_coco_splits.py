...
from ml.data.coco_dataset_splitter import create_maxsight_splits_from_coco, split_coco_dataset
...
def main():
    ...
    parser.add_argument(
        "--coco_dir", type=Path, required=True, help="Root directory containing COCO dataset"
    )
    ...
    parser.add_argument("--train_split", type=float, default=0.7, help="Fraction for training")

    parser.add_argument("--val_split", type=float, default=0.15, help="Fraction for validation")

    parser.add_argument("--test_split", type=float, default=0.15, help="Fraction for testing")
    ...
    if args.train_samples is not None and args.val_samples is not None:
        ...
        print(
            f"Splits: Train={args.train_split:.1%}, Val={args.val_split:.1%}, Test={args.test_split:.1%}"
        )
    print("=" * 60)
    print()
