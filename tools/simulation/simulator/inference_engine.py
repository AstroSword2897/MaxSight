...
from ml.utils.error_handling import HeadExecutionManager
...
class InferenceMetrics:
    ...
    def add_inference(
        self, latency_ms: float, uncertainty: float, success: bool, fallback_used: bool
    ):
...
class InferenceEngine:
    ...
    def infer(
        self, image: torch.Tensor, audio_features: torch.Tensor | None = None
        ...
        try:
            ...
            self.metrics.add_inference(
                latency_ms=latency_ms, uncertainty=uncertainty, success=True, fallback_used=False
            )
            ...
            self.metrics.add_inference(
                latency_ms=latency_ms, uncertainty=1.0, success=False, fallback_used=True
            )
    ...
    def _check_circuit_breaker(self):
        ...
        if avg_uncertainty > self.circuit_breaker_config.avg_uncertainty_threshold:
            if self.state == InferenceState.STABLE:
                logger.warning(
                    f"Avg uncertainty {avg_uncertainty:.2f} exceeds threshold, DEGRADING"
                )
                self.state = InferenceState.DEGRADED
