...
class TherapySimulator:
    ...
    def __init__(self, config: dict[str, Any] | None = None, model=None):
    ...
    def start_simulation(self, video_source: str | None = None):
        """Start simulation."""
    ...
    def _generate_summary(self) -> dict[str, Any]:
        ...
        for log in self.logs:
            if "error" in log.get("model_output", {}):
                errors.append({"frame": log["frame_number"], "error": log["model_output"]["error"]})
    ...
    def save_logs(self, filepath: str):
        ...
        with open(filepath, "w") as f:
            json.dump(
                {"config": self.config, "logs": self.logs, "summary": self._generate_summary()},
                f,
                indent=2,
            )
