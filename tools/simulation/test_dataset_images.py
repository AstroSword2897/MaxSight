...
class DatasetImageTester:
    ...
    def find_dataset_images(self, dataset_dirs: list[Path], max_images: int = 50) -> list[Path]:
    ...
    def process_image(self, image_path: Path) -> dict[str, Any]:
        ...
        try:
                ...
                scene_detections.append(
                    {
                        ...
                        "box": torch.tensor(
                            det.get("box", [0.5, 0.5, 0.1, 0.1]), dtype=torch.float32
                        ),
            ...
            path_info = self.path_planner.plan_path(
                detections=detections, target_direction="forward"
            )
            ...
            scheduled_outputs = self.scheduler.schedule_outputs(
                detections=detections, model_outputs=model_outputs, timestamp=time.time()
            )

            # Therapy integration (create a task from detections)
            if detections:
                target_objects = [
                    d.get("class_idx", 0) for d in detections[:3]
                ]  # First 3 detections.
            ...
            overlay_info = self.overlay_engine.add_halo(
                center=(0.5, 0.5), radius=50.0, intensity=0.3
            )
            ...
            if scheduled_outputs:
                self.haptic_feedback.trigger(
                    self.haptic_feedback.HapticPattern.MICRO_PULSE, intensity=0.5
                )
            ...
            result = {
                ...
                "urgency_level": outputs.get("urgency_scores", torch.zeros(1, 4))
                .argmax(dim=1)
                .item()
                if "urgency_scores" in outputs
                else 0,
                "uncertainty": outputs.get("uncertainty", torch.zeros(1, 1)).mean().item()
    ...
    def _extract_detections(
        self, outputs: dict[str, torch.Tensor], confidence_threshold: float = 0.3
    ) -> list[dict[str, Any]]:
        """Extract detections from model outputs."""
        detections = []

        if (
            "classifications" not in outputs
            or "boxes" not in outputs
            or "objectness" not in outputs
        ):
            return detections
    ...
    def run_tests(
        self, image_paths: list[Path], conditions: list[str] | None = None
    ) -> dict[str, Any]:
        """Run tests on multiple images."""
        # Convert conditions to List[Optional[str]].
        if conditions is not None:
            test_conditions: list[str | None] = list(
                conditions
            )  # List[str] -> List[Optional[str]].
        else:
        ...
        for condition in test_conditions:
            ...
            for i, image_path in enumerate(image_paths, 1):
                ...
                if result["success"]:
                    print(
                        f"OK ({result['processing_time_ms']:.1f}ms, {result['num_detections']} detections)"
                    )
                else:
