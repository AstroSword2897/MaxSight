...
from ml.middleware.error_sanitizer import log_error, sanitize_error
...
from .degraded_modes import DegradedMode, DegradedState
from .metrics import get_health_status, metrics
from .output_hierarchy import OutputAuthority, OutputAuthorityManager, OutputRequest
from .priority_queue import MessagePriority, PriorityQueue
from .rate_limiter import GlobalRateLimiter, RateLimiter
from .structured_logging import get_component_logger, setup_structured_logging
from .utils import (
...
class MaxSightCore:
    ...
    def __init__(self, device: str | None = None):
        ...
        try:
            ...
            if not checkpoint_path:
                default_glaucoma = Path(
                    "/content/drive/MyDrive/MaxSight/checkpoints_glaucoma/best_model.pt"
                )
    ...
    @classmethod
    def get_instance(cls, device: str | None = None) -> "MaxSightCore":
...
class MaxSightSession:
    ...
    def __init__(self, session_id: str, core: MaxSightCore, output_mode: OutputMode | None = None):
        ...
        self.preprocessor: ImagePreprocessor | None = None
        ...
        self._last_therapy_perception: dict[str, Any] | None = None
    ...
    def set_therapy_output_preferences(
        ...
        with self.lock:
            ...
            if (
                preferred == "audio"
                and not self.therapy_voice_enabled
                and self.therapy_haptic_enabled
            ):
                preferred = "haptic"
            if (
                preferred == "haptic"
                and not self.therapy_haptic_enabled
                and self.therapy_voice_enabled
            ):
                preferred = "audio"

            # Re-create the therapy engine so InterventionGenerator respects the channel preference.
            self.therapy_engine = TherapyEngine(
                config=TherapyEngineConfig(preferred_channel=preferred)
            )
        ...
        with self.lock:
            ...
            while not self.voice_queue.empty():
                try:
                    ...
                    session_logger.warning(
                        f"Error flushing voice queue during abort: {str(e)}",
            ...
            while not self.haptic_queue.empty():
                try:
                    ...
                    session_logger.warning(
                        f"Error flushing haptic queue during abort: {str(e)}",
                        session_id=self.session_id,
                    )
            ...
            try:
                ...
                session_logger.warning(
                    f"Error stopping voice feedback during abort: {str(e)}",
                    session_id=self.session_id,
                )
            try:
                ...
                session_logger.warning(
                    f"Error stopping haptic feedback during abort: {str(e)}",
                    session_id=self.session_id,
                )
            ...
            while self._voice_worker_running and not self._aborted:
                try:
                    ...
                    if not self.degraded_state.is_degraded(DegradedMode.AUDIO_UNAVAILABLE):
                        try:
                            ...
                            session_logger.error(
                                f"Voice hardware error: {str(e)}", session_id=self.session_id
                            )
                            if consecutive_failures >= max_failures:
                                self.degraded_state.set_degraded(
                                    DegradedMode.AUDIO_UNAVAILABLE, f"Hardware failure: {str(e)}"
                                )
                                break
                            ...
                            session_logger.error(
                                f"Voice processing error: {str(e)}", session_id=self.session_id
                            )
                            consecutive_failures += 1
                            if consecutive_failures >= max_failures:
                                self.degraded_state.set_degraded(
                                    DegradedMode.AUDIO_UNAVAILABLE, str(e)
                                )
                                break
                    else:
                        session_logger.warning(
                            "Voice output suppressed - audio degraded", session_id=self.session_id
                        )
                except Exception as e:
                    ...
                    if not isinstance(e, (Empty, TimeoutError, AttributeError)):
                        session_logger.error(
                            f"Voice queue error: {str(e)}", session_id=self.session_id
                        )
                ...
                try:
                    ...
                    if not self.degraded_state.is_degraded(DegradedMode.HAPTIC_UNAVAILABLE):
                        try:
                            ...
                            consecutive_failures += 1
                            session_logger.error(
                                f"Haptic hardware error: {str(e)}", session_id=self.session_id
                            )
                            if consecutive_failures >= max_failures:
                                self.degraded_state.set_degraded(
                                    DegradedMode.HAPTIC_UNAVAILABLE, f"Hardware failure: {str(e)}"
                                )
                                break
                            time.sleep(backoff_seconds)
                            backoff_seconds = min(backoff_seconds * 2, 5.0)
                            ...
                            session_logger.error(
                                f"Haptic processing error: {str(e)}", session_id=self.session_id
                            )
                            consecutive_failures += 1
                            if consecutive_failures >= max_failures:
                                self.degraded_state.set_degraded(
                                    DegradedMode.HAPTIC_UNAVAILABLE, str(e)
                                )
                                break
                    else:
                        session_logger.warning(
                            "Haptic output suppressed - haptic degraded", session_id=self.session_id
                        )
                except Exception as e:
                    # Queue errors - log but continue. Empty exception is expected on timeout, don't log as error.
                    from queue import Empty
                    ...
                    if not isinstance(e, (Empty, TimeoutError, AttributeError)):
                        session_logger.error(
                            f"Haptic queue error: {str(e)}", session_id=self.session_id
                        )
                    time.sleep(0.1)  # Brief pause on error.

        self.voice_thread = threading.Thread(
            target=voice_worker, daemon=True, name=f"Voice-{self.session_id}"
        )
        self.haptic_thread = threading.Thread(
            target=haptic_worker, daemon=True, name=f"Haptic-{self.session_id}"
        )
    ...
    def _postprocess_outputs(
        self, outputs: dict[str, Any], confidence_threshold: float = 0.3
    ) -> list[dict[str, Any]]:
    ...
    def _run_inference(
        self, image_tensor: torch.Tensor, audio_features: np.ndarray | None = None
        ...
        with INFERENCE_SEMAPHORE:
            ...
            with torch.no_grad():  # Prevents graph construction and memory leaks.
                if audio_features is not None:
                    audio_tensor = (
                        torch.from_numpy(audio_features).unsqueeze(0).to(self.core.device)
                    )
    ...
    def _build_perception_for_therapy(
        ...
        urgency_level = (
            int(urgency_scores.argmax(dim=1).item())
            if isinstance(urgency_scores, torch.Tensor)
            else 0
        )
        ...
        navigation_difficulty = min(
            1.0, 0.6 * uncertainty + 0.3 * scene_complexity + 0.1 * (1.0 - temporal_consistency)
        )
    ...
    def _therapy_actions_to_feedback(self, therapy_actions: list[Any]) -> dict[str, Any] | None:
    ...
    def _queue_therapy_actions(self, therapy_actions: list[Any]) -> None:
        ...
        if self._last_therapy_perception is not None:
            temporal_reliability = float(
                self._last_therapy_perception.get("temporal_reliability", 1.0)
            )
        ...
        for action in therapy_actions:
            ...
            if channel == "audio":
                ...
                pattern = (
                    ...
                    if intervention_type in long_interventions
                    else HapticPattern.MICRO_PULSE
                )
                if priority_val >= 70:
                    pr = MessagePriority.HIGH
                elif priority_val >= 60:
    ...
    def _run_ocr(self, image: Image.Image, outputs: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            ocr_results = self.core.ocr.process_image_for_ocr(
                image=image, text_scores=text_scores[0], boxes=boxes[0]
            )
        except Exception as e:
            session_logger.error(f"OCR processing failed: {str(e)}", session_id=self.session_id)
    ...
    def _generate_description(
        ...
        scene_description = self.core.description_gen.generate_scene_description(
            detections=scene_detections, urgency_score=urgency_level
        )
    ...
    def _update_memory(self, detections_list: list[dict[str, Any]]) -> None:
        ...
        if self._spatial_memory_count >= config.max_spatial_memory_entries:
            ...
            try:
                ...
                memory_size = (
                    len(self.spatial_memory.memory) if hasattr(self.spatial_memory, "memory") else 0
                )
                if memory_size >= config.max_spatial_memory_entries:
                    ...
                    self.degraded_state.set_degraded(
                        DegradedMode.MEMORY_FULL,
                        f"Memory at capacity, pruned {prune_count} entries",
                    )
            except Exception as e:
                session_logger.error(
                    f"Error pruning spatial memory: {str(e)}", session_id=self.session_id
                )
        ...
        if spatial_detections:
            try:
                self.spatial_memory.update(detections=spatial_detections, timestamp=time.time())
                # Update resource tracking.
                self._spatial_memory_count = (
                    len(self.spatial_memory.memory)
                    if hasattr(self.spatial_memory, "memory")
                    else self._spatial_memory_count + len(spatial_detections)
                )
            except Exception as e:
                session_logger.error(
                    f"Error updating spatial memory: {str(e)}", session_id=self.session_id
                )
                self.degraded_state.set_degraded(DegradedMode.MEMORY_FULL, str(e))

    def _plan_path(self, detections_list: list[dict[str, Any]]) -> dict[str, Any] | None:
        """Plan navigation path if navigation scenario."""
        path_info = None
        if self.current_scenario == "navigation":
            path_result = self.path_planner.plan_path(
                detections=detections_list, target_direction="forward"
            )
    ...
    def _schedule_outputs(
        self, detections_list: list[dict[str, Any]], outputs: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        scheduled_outputs = self.core.scheduler.schedule_outputs(
            detections=detections_list, model_outputs=model_outputs, timestamp=time.time()
        )
        if isinstance(scheduled_outputs, list):
            return {"outputs": scheduled_outputs, "count": len(scheduled_outputs)}
        return (
            scheduled_outputs
            if isinstance(scheduled_outputs, dict)
            else {"outputs": scheduled_outputs}
        )
    ...
    def _queue_outputs(
        self, scene_description: str, outputs: dict[str, Any], detections_list: list[dict[str, Any]]
        ...
        if self.output_mode == OutputMode.PATIENT and overall_confidence < min_confidence:
            ...
            request = OutputRequest(
                authority=OutputAuthority.DESCRIPTIVE_NARRATION, content=neutral_message, priority=0
            )
        ...
        if urgency_level >= warning_threshold and urgency_confidence >= min_critical_confidence:
            warning_message = "Warning: High urgency detected"
            request = OutputRequest(
            ...
            if self.output_authority.request_output(request):
                ...
                haptic_patterns.append(
                    {"pattern": "long_pulse", "intensity": config.haptic_intensity_high}
                )
            ...
            haptic_patterns.append(
                {"pattern": "micro_pulse", "intensity": config.haptic_intensity_low}
            )
    ...
    def process_frame(
        ...
        with self.lock:  # Per-session locking.
            ...
            if self._aborted:
                session_logger.warning(
                    "Frame processing rejected - session aborted", session_id=self.session_id
                )
            ...
            if self._spatial_memory_count >= config.max_spatial_memory_entries:
                self.degraded_state.set_degraded(
                    DegradedMode.MEMORY_FULL, f"Memory at capacity: {self._spatial_memory_count}"
                )
                session_logger.warning(
                    "Spatial memory at capacity",
                    session_id=self.session_id,
            ...
            try:
                ...
                if self.temporal_enabled:
                    ...
                    if len(self._temporal_frame_buffer) >= self.temporal_window_frames:
                        should_use_temporal = self.temporal_stride <= 1 or (
                            self.stats.get("frames_processed", 0) % self.temporal_stride == 0
                        )
                ...
                tracker.end_stage()
                session_logger.error(
                    f"Image preprocessing failed: {str(e)}", session_id=self.session_id
                )
            ...
            try:
                ...
                tracker.end_stage()
                session_logger.error(
                    f"Model inference failed: {str(e)}", session_id=self.session_id
                )
                self.degraded_state.set_degraded(DegradedMode.VISION_UNSTABLE, str(e))
            ...
            contract_debug["forward_surface_contract"] = {
                "has_retrieval_artifacts": any(
                    k in outputs for k in ("retrieval_results", "distances", "indices", "retrieval")
                ),
                ...
                if isinstance(outputs.get("classifications"), torch.Tensor)
                and outputs["classifications"].numel() > 0
                else None,
            }
            ...
            rag_surface_ok = (
                ...
                and not contract_debug["forward_surface_contract"][
                    "has_scene_description_in_model_outputs"
                ]
            )
            temporal_ok = (not contract_debug["temporal"]["used_sequence"]) or contract_debug[
                "temporal"
            ]["sliced_to_batch1"]
            ...
            detections_list = self._postprocess_outputs(
                outputs, confidence_threshold=config.confidence_threshold
            )
            if not hasattr(self, "priority_filter"):
                self.priority_filter = PriorityBudgetFilter(
                    max_alerts_per_frame=config.max_alerts_per_frame
                )
                self.alert_cooldown = AlertCooldownFilter(
                    cooldown_frames=config.alert_cooldown_frames
                )
            ...
            therapy_feedback: dict[str, Any] | None = None
            if self.session_active and detections_list and not self._aborted:
                ...
                perception_for_therapy = self._build_perception_for_therapy(
                    detections_list, outputs
                )
                ...
                therapy_future = self._pipeline_executor.submit(
                    self.therapy_engine.update, perception_for_therapy
                )
            ...
            if therapy_future is not None:
                ...
                contract_debug["therapy_delivery"]["generated_channels"] = [
                    getattr(a, "channel", None)
                    for a in therapy_actions
                    if getattr(a, "channel", None) is not None
                ]
                ...
                if self._last_therapy_perception is not None:
                    contract_debug["therapy_delivery"]["temporal_reliability"] = (
                        self._last_therapy_perception.get("temporal_reliability")
                    )
                    contract_debug["therapy_delivery"]["flicker_risk"] = (
                        self._last_therapy_perception.get("flicker_risk")
                    )
                ...
                contract_debug["checks"]["therapy_channel_policy_ok"] = all(
                    (ch in allowed_channels)
                    for ch in contract_debug["therapy_delivery"].get("generated_channels", [])
            ...
            try:
                overlay_image_b64 = self._render_overlay(
                    image, detections_list, ocr_results, path_info
                )
            except Exception as exc:
                session_logger.warning(
            ...
            if not self._aborted and not skip_non_critical:
                ...
                voice_announcements, haptic_patterns = self._queue_outputs(
                    scene_description, outputs, detections_list
                )
            ...
            self.stats["avg_latency_ms"] = (
                self.stats["total_inference_time"] / self.stats["frames_processed"] * 1000
            )
    ...
    def _extract_reasoning_trace(
        self, outputs: dict[str, Any], detections_list: list[dict[str, Any]]
    ) -> dict[str, Any]:
        ...
        if detections_list:
            top_detections = sorted(
                detections_list, key=lambda x: x.get("confidence", 0), reverse=True
            )[:5]
            trace["decision_path"] = [
                {
                    "step": i + 1,
    ...
    def _extract_final_judgment(
        self, outputs: dict[str, Any], detections_list: list[dict[str, Any]]
    ) -> dict[str, Any]:
        """Extract final weighted judgment with confidence scores."""
        urgency_scores = outputs.get("urgency_scores", torch.zeros(1, 4))
        urgency_level = (
            int(urgency_scores.argmax(dim=1).item()) if urgency_scores.numel() > 0 else 0
        )
        urgency_confidence = (
            float(torch.softmax(urgency_scores, dim=-1)[0].max().item())
            if urgency_scores.numel() > 0
            else 0.0
        )
    ...
    def _shape_response(
        ...
        path_info: dict[str, Any] | None,
        therapy_feedback: dict[str, Any] | None,
        overlay_image_b64: str | None,
...
class SessionRegistry:
    ...
    def __init__(self):
        ...
        self.lock = threading.Lock()
        self.core: MaxSightCore | None = None
    ...
    def get_core(self, device: str | None = None) -> MaxSightCore:
    ...
    def create_session(
        self, output_mode: OutputMode = OutputMode.PATIENT, device: str | None = None
    ) -> str:
    ...
    def get_session(self, session_id: str) -> MaxSightSession | None:
...
class MaxSightSimulator:
    ...
    def __init__(self, device: str | None = None, output_mode: OutputMode = OutputMode.PATIENT):
        ...
        logger.warning(
            "Using deprecated MaxSightSimulator class. Consider migrating to MaxSightSession."
        )
    ...
    def _queue_outputs(
        ...
        if urgency_scores.numel() > 0:
            ...
            if urgency_level >= warning_threshold:
                # Queue format: (message, priority)
                self.voice_queue.put(("Warning: High urgency detected", MessagePriority.CRITICAL))
                voice_announcements.append("Warning: High urgency detected")
    ...
    def process_frame(
        self, image: Image.Image, audio_features: np.ndarray | None = None
    ) -> dict[str, Any]:
        """Process a single frame through the complete MaxSight pipeline."""
        start_time = time.perf_counter()
        tracker = PipelineLatencyTracker()
            ...
            try:
                ...
                baseline = {
                    ...
                    "num_detections": result.get(
                        "num_detections", stats.get("total_detections", 0)
                    ),
                    "num_text_regions": result.get("num_text_regions", 0),
                    ...
                    "inference_time_ms": result.get(
                        "inference_time_ms", stats.get("avg_latency_ms", 0)
                    ),
        ...
        try:
            if frames_mode and frames is not None:
                ...
                for i, frame_image in enumerate(frames):
                    ...
                    last_result = session.process_frame(
                        frame_image, audio_features, frame_id=seq_frame_id
                    )
            ...
            api_logger.error(
                f"Error processing frame in session {session.session_id}: {e}", exc_info=True
            )
            return jsonify(
                {"error": f"Error processing image: {str(e)}", "error_type": type(e).__name__}
            ), 500
...
def api_session_start():
    ...
    try:
        ...
        with session.lock:
            ...
            session.session_manager.start_session()
        return jsonify({"status": "session_started", "session_id": session.session_id})
    except ValueError as e:
        return jsonify({"error": str(e)}), 400
...
def api_set_mode():
    ...
    try:
        ...
        return jsonify(
            {"status": "mode_updated", "session_id": session.session_id, "output_mode": mode_str}
        )
    except ValueError as e:
        return jsonify({"error": str(e)}), 400
...
def api_sample():
    ...
    try:
        ...
        if dataset not in dataset_dirs:
            return jsonify(
                {
                    "error": f"Invalid dataset: {dataset}. Must be one of: {list(dataset_dirs.keys())}"
                }
            ), 400
        ...
        try:
            ...
            return jsonify(
                {"error": f"Failed to load sample image: {str(e)}", "image_path": str(image_path)}
            ), 400
        ...
        try:
            ...
            api_logger.error(
                f"Error processing sample image in session {session.session_id}: {e}", exc_info=True
            )
            return jsonify(
                {
        ...
        return jsonify(
            {"error": f"Error in sample endpoint: {str(e)}", "error_type": type(e).__name__}
        ), 500
...
def rate_limit_error(error):
    ...
    return jsonify({"error": "Rate Limit Exceeded", "message": str(error), "status_code": 429}), 429


@app.errorhandler(ValidationError)
def validation_error(error):
    """Handle validation errors."""
    return jsonify({"error": "Validation Error", "message": str(error), "status_code": 400}), 400


@app.errorhandler(SessionNotFoundError)
def session_not_found_error(error):
    """Handle session not found errors."""
    return jsonify({"error": "Session Not Found", "message": str(error), "status_code": 404}), 404
...
if __name__ == "__main__":
    ...
    logger.info(
        "Runtime mode: %s (set MAXSIGHT_RUNTIME=production for hardened defaults)",
        get_runtime_mode().value,
    )
    logger.info(
        f"Multi-user mode: {'ENABLED' if config.multi_user_enabled else 'DISABLED (single-user demo)'}"
    )
    ...
    if actual_port != _preferred:
        logger.warning(
            "Port %s is in use; bound to %s instead (set MAXSIGHT_PORT or MAXSIGHT_STRICT_PORT=1).",
            _preferred,
            actual_port,
        )
