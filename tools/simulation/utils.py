...
def preprocess_image(
    image: Image.Image, preprocessor: Any | None, device: torch.device
) -> torch.Tensor:
...
def postprocess_outputs(
    model: Any, outputs: dict[str, Any], confidence_threshold: float = 0.3
) -> list[dict[str, Any]]:
...
def prepare_scene_detections(
    detections_list: list[dict[str, Any]], urgency_level: int
) -> list[dict[str, Any]]:
