...
def _load_checkpoint_file(path: Path, map_location: torch.device):
    ...
    try:
        ...
        if os.getenv("MAXSIGHT_ALLOW_LEGACY_PICKLE") == "1":
            logger.warning(
                "Using legacy pickle load for %s (MAXSIGHT_ALLOW_LEGACY_PICKLE=1): %s", path, e
            )
...
class ComprehensiveSimulator:
    ...
    def process_image(
        ...
        if show_overlay or save_output:
            vis_image = self._create_visualization(
                image, detections[0] if detections else [], outputs
            )

            if save_output:
                if output_path is None:
                    output_path = str(
                        Path(image_path).parent / f"{Path(image_path).stem}_output.jpg"
                    )
        ...
        if temporal_window_frames <= 1:
            while self.is_running and cap.isOpened():
                ...
                vis_frame = self._create_visualization(
                    frame_pil, result["detections"], result["outputs"]
                )
            ...
            try:
                while self.is_running and cap.isOpened():
                    ...
                    assert image_tensor.dim() == 5, (
                        "Expected [1, T, C, H, W] for temporal inference."
                    )
                    assert image_tensor.shape[1] == temporal_window_frames, (
                        "Temporal window length mismatch."
                    )
                    ...
                    if "stage_a_completed" in outputs:
                        assert outputs["stage_a_completed"] is True, (
                            "Stage A did not complete successfully."
                        )
                    if (
                        "stage_b_completed" in outputs
                        and outputs.get("skip_stage_b_reason") is None
                    ):
                        # When Stage B isn't skipped, stage_b_completed must be True.
                        assert outputs["stage_b_completed"] is True, (
                            "Stage B did not complete successfully."
                        )
                    ...
                    detections_per_frame = self.model.get_detections(
                        outputs, confidence_threshold=0.3
                    )
                    assert len(detections_per_frame) == temporal_window_frames, (
                        "Detection list length mismatch."
                    )
                    ...
                    outputs_last = {
                        "classifications": outputs["classifications"][end_frame_idx]
                        .detach()
                        .cpu()
                        .numpy(),
                        "boxes": outputs["boxes"][end_frame_idx].detach().cpu().numpy(),
                        "objectness": outputs["objectness"][end_frame_idx].detach().cpu().numpy(),
                        "urgency_scores": outputs["urgency_scores"][end_frame_idx]
                        .detach()
                        .cpu()
                        .numpy()
                        if isinstance(outputs.get("urgency_scores"), torch.Tensor)
                        else None,
                        "distance_zones": outputs["distance_zones"][end_frame_idx]
                        .detach()
                        .cpu()
                        .numpy(),
                        "scene_embedding": outputs["scene_embedding"][end_frame_idx]
                        .detach()
                        .cpu()
                        .numpy()
                        if isinstance(outputs.get("scene_embedding"), torch.Tensor)
                        else None,
                    }
                    ...
                    if save_frames and output_path:
                        frame_output_path = (
                            Path(output_path).parent / f"frame_{end_frame_number:06d}.jpg"
                        )
                        vis_frame.save(str(frame_output_path))
    ...
    def process_image_frame(self, image: Image.Image, frame_number: int = 0) -> dict[str, Any]:
    ...
    def _create_visualization(
        self, image: Image.Image, detections: list[dict], outputs: dict[str, np.ndarray]
    ) -> Image.Image:
    ...
    def save_session(self, filepath: str):
        ...
        session_data = {
            "config": {"condition_mode": self.condition_mode, "device": str(self.device)},
...
def main():
    ...
    parser.add_argument(
        "--temporal-window", type=int, default=1, help="Temporal window size T for video sequencing"
    )
    parser.add_argument(
        "--temporal-stride", type=int, default=1, help="Emit stride for temporal windows"
    )
    parser.add_argument(
        "--preprocess-workers",
        type=int,
        default=4,
        help="CPU workers for preprocessing temporal windows",
    )
    ...
    simulator = ComprehensiveSimulator(
        condition_mode=args.condition, device=args.device, verbose=not args.quiet
    )
    ...
    if input_path.is_file():
        if input_path.suffix.lower() in [".jpg", ".jpeg", ".png", ".bmp"]:
            ...
            result = simulator.process_image(
                str(input_path), save_output=args.output is not None, output_path=args.output
            )
        ...
        result = simulator.process_directory(
            str(input_path), output_dir=args.output, max_images=args.max_images
        )
    else:
