...
class RetrievalIntegration:
    ...
    def load_index(self, index_path: str):
        ...
        try:
            ...
            self.stage1 = Stage1ANN(index=self.index, top_k=10)
    ...
    def retrieve_similar_scenes(self, query_embedding: torch.Tensor, top_k: int = 5) -> list[dict]:
        ...
        for i, (idx, dist) in enumerate(zip(indices[0][:top_k], distances[0][:top_k])):
            results.append({"index": int(idx), "distance": float(dist), "rank": i + 1})

        return results

    def enhance_with_retrieval(
        self, model_outputs: dict[str, torch.Tensor], images: torch.Tensor
