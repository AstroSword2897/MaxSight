...
class QuantizationValidator:
    ...
    def compute_relative_error(
        self, fp32_out: torch.Tensor, int8_out: torch.Tensor
    ) -> dict[str, float]:
    ...
    def validate_classification_head(
        ...
        if fp32_logits.shape != int8_logits.shape:
            raise ValueError(
                f"Shape mismatch: FP32 {fp32_logits.shape} vs INT8 {int8_logits.shape}"
            )
        ...
        results = {**{f"logits_{k}": v for k, v in output_error.items()}}
    ...
    def validate_bbox_head(
        ...
        targets_bbox: torch.Tensor | None = None,
    ) -> dict[str, Any]:
        ...
        if fp32_bbox.shape != int8_bbox.shape:
            return {
                "_bbox_error": f"Shape mismatch: FP32 {fp32_bbox.shape} vs INT8 {int8_bbox.shape}"
            }
        ...
        if targets_bbox is not None:
            ...
            def compute_iou_center_format(pred, target):
                ...
                target_area = (target_corners[:, 2] - target_corners[:, 0]) * (
                    target_corners[:, 3] - target_corners[:, 1]
                )
    ...
    def validate_embedding_head(
        self, fp32_embed: torch.Tensor, int8_embed: torch.Tensor
    ) -> dict[str, float]:
    ...
    def validate_urgency_head(
        ...
        if fp32_urgency.shape != int8_urgency.shape:
            raise ValueError(
                f"Shape mismatch: FP32 {fp32_urgency.shape} vs INT8 {int8_urgency.shape}"
            )
    ...
    def validate_objectness_head(
        self, fp32_obj: torch.Tensor, int8_obj: torch.Tensor
    ) -> dict[str, float]:
                ...
                try:
                    ...
                    if isinstance(fp32_out, dict):
                        ...
                        if "classifications" in fp32_out:
                            class_targets = (
                                targets.get("labels") if isinstance(targets, dict) else None
                            )
                        ...
                        if "boxes" in fp32_out:
                            bbox_targets = (
                                targets.get("boxes") if isinstance(targets, dict) else None
                            )
                            ...
                            try:
                                metrics = self.validate_bbox_head(
                                    fp32_out["boxes"], int8_out["boxes"], bbox_targets
                                )
                        ...
                        if "scene_embedding" in fp32_out:
                            try:
                                metrics = self.validate_embedding_head(
                                    fp32_out["scene_embedding"], int8_out["scene_embedding"]
                                )
                        ...
                        if "urgency_scores" in fp32_out:
                            urgency_targets = (
                                targets.get("urgency") if isinstance(targets, dict) else None
                            )
                        ...
                        if "objectness" in fp32_out:
                            try:
                                metrics = self.validate_objectness_head(
                                    fp32_out["objectness"], int8_out["objectness"]
                                )
                        ...
                        if (batch_idx + 1) % 10 == 0 or (batch_idx + 1) == total_batches:
                            ...
                            print(
                                f"  Processed {batch_idx + 1}/{total_batches} batches ({progress:.1f}%)"
                            )
...
class ModelBenchmark:
    ...
    def compare_models(
        ...
        speedup = (
            fp32_bench["mean_ms"] / int8_bench["mean_ms"] if int8_bench["mean_ms"] > 0 else 0.0
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--fp32-model", type=str, required=True, help="Path to FP32 model state dict"
    )
    parser.add_argument(
        "--int8-model", type=str, required=True, help="Path to INT8 model state dict"
    )
    parser.add_argument(
        "--model-file", type=str, required=True, help="Python file with build_model() function"
    )
    parser.add_argument("--data-dir", type=str, required=True, help="Test data directory")
    parser.add_argument("--batch-size", type=int, default=16, help="Batch size")
    parser.add_argument("--device", type=str, default="cpu", help="Device (cpu/cuda)")
    parser.add_argument(
        "--output-file", type=str, default="validation_results.json", help="Output JSON file"
    )
    ...
    test_data = TensorDataset(torch.randn(50, 3, 224, 224), torch.randint(0, 10, (50,)))
