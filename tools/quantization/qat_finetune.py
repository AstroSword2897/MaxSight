    ...
    for name, module in model.named_modules():
        try:
            ...
            for i in range(len(children) - 2):
                ...
                if (
                    isinstance(m_a, nn.Conv2d)
                    and isinstance(m_b, (nn.BatchNorm2d, nn.BatchNorm1d))
                    and isinstance(m_c, (nn.ReLU, nn.ReLU6))
                ):
                    full_name = f"{name}.{a}" if name else a
                    patterns.append(
                        [f"{full_name}", f"{name}.{b}" if name else b, f"{name}.{c}" if name else c]
                    )
        except Exception:
            continue
...
class QATTrainer:
    ...
    def __init__(
        ...
        self.history = {"train_loss": [], "val_loss": [], "val_metrics": [], "lr": []}
    ...
    def prepare_model_for_qat(self) -> nn.Module:
        ...
        model_qat.qconfig = qconfig  # type: ignore
    ...
    def train_epoch(self, model: nn.Module, optimizer, epoch: int) -> float:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if isinstance(targets, dict):
                targets = {
                    k: v.to(self.device) if torch.is_tensor(v) else v for k, v in targets.items()
                }
    ...
    def validate(self, model: nn.Module, epoch: int) -> dict[str, float]:
        ...
        metrics = {"loss": avg_loss, "num_batches": num_batches}
    ...
    def train(self) -> dict[str, Any]:
        ...
        if self.freeze_backbone:
            ...
            # Only optimize heads.
            optimizer = AdamW(self.head_params, lr=self.lr, weight_decay=1e-4)
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-dir", type=str, required=True, help="Data directory for train/val")
    parser.add_argument("--epochs", type=int, default=5, help="Number of QAT epochs")
    parser.add_argument("--lr", type=float, default=1e-5, help="Learning rate")
    parser.add_argument("--batch-size", type=int, default=32, help="Batch size")
    parser.add_argument("--device", type=str, default="cuda", help="Device (cuda/cpu)")
    parser.add_argument(
        "--output-dir", type=str, default="./artifacts/qat", help="Output directory"
    )
    parser.add_argument(
        "--backend",
        type=str,
    ...
    parser.add_argument("--num-classes", type=int, default=48, help="Number of classes")
    ...
    val_data = TensorDataset(torch.randn(20, 3, 224, 224), torch.randint(0, 10, (20,)))
