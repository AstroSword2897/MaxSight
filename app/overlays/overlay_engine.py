...
class OverlayEngine:
    ...
    def add_halo(
        self, center: tuple[float, float], radius: float, intensity: float = 0.3
    ) -> dict[str, Any]:
    ...
    def add_edge_glow(
        self, edges: list[tuple[float, float]], width: float = 2.0, intensity: float = 0.4
    ) -> dict[str, Any]:
    ...
    def add_depth_fog(self, depth_map: np.ndarray, near_threshold: float = 0.3) -> dict[str, Any]:
    ...
    def add_motion_trace(
        self, path: list[tuple[float, float]], width: float = 3.0
    ) -> dict[str, Any]:
    ...
    def add_gaze_indicator(
        self, position: tuple[float, float], size: float = 10.0
    ) -> dict[str, Any]:
    ...
    def add_guidance_arrow(
        self, start: tuple[float, float], end: tuple[float, float], width: float = 5.0
    ) -> dict[str, Any]:
    ...
    def create_overlay(
        ...
        urgency_scores: np.ndarray | None = None,
        ...
        for det in detections:
            ...
            if len(bbox) == 4:
                ...
                cv2.rectangle(
                    img_array, (x1, y1 - label_size[1] - 10), (x1 + label_size[0], y1), color, -1
                )
                cv2.putText(
                    img_array,
                    label,
    ...
    def render_overlays(self, base_image: np.ndarray) -> np.ndarray:
        ...
        for overlay in self.active_overlays:
            ...
            if overlay_type == "halo":
                ...
                if edges:
                    points = np.array(
                        [(int(x * result.shape[1]), int(y * result.shape[0])) for x, y in edges],
                        dtype=np.int32,
                    )
